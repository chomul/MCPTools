...
3. **[배경 제거 + 격자 검출]** — 흰 배경을 외곽 flood-fill로 제거하고 AI가 그린 격자선을 검출합니다. **이 단계에서는 아무것도 기록하지 않습니다.**
   - flood-fill은 이미지 네 외곽에서 시작해 ① 이웃과의 색 차가 허용 오차 이내이거나, ② 근사 흰색이거나, ③ **밝은 무채색이면서 지금 픽셀보다 밝은** 픽셀로 번집니다. ③은 시트 가장자리 격자선(회색)과 흰 배경 사이의 안티에일리어싱 계단(예: 159 → 216 → 252)을 넘기 위한 조건으로, 이게 없으면 그 1픽셀이 장벽이 되어 배경이 통째로 남습니다. **밝아지는 방향으로만** 허용하므로 배경에서 더 어두운 캐릭터 쪽으로는 번지지 않고, 유채색 픽셀은 어떤 경로로도 배경이 되지 않아 글로우 이펙트가 보존됩니다.
