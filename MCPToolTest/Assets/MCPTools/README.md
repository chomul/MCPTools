...
- URL 끝의 **`#v0.3.0`은 git 태그**입니다. 태그를 붙이면 해당 버전으로 **고정**되어 팀원 모두가 같은 버전을 받습니다.
- 태그를 **생략하면 기본 브랜치의 HEAD**를 받으므로, 설치 시점에 따라 서로 다른 버전이 될 수 있습니다. 팀 프로젝트에서는 태그를 붙이는 것을 권장합니다.
- 업데이트하려면 프로젝트의 `Packages/manifest.json`에서 이 패키지의 URL 태그를 새 버전으로 바꾸거나(예: `#v0.1.0` → `#v0.3.0`), Package Manager에서 **패키지를 제거한 뒤 새 URL로 다시 추가**합니다. 변경 이력은 [CHANGELOG.md](CHANGELOG.md)를 참고하세요.
...
예외로 **읽기 전용 진단 도구 3개**(`mcptools_ping`, `mcptools_status`, `mcptools_list_candidates`)는 언제든 호출할 수 있습니다. `list_candidates`는 생성 Job의 완료를 확인하는 폴링 경로라 막지 않습니다.
...
- 읽은 값이 도구가 아는 버전보다 **크면 콘솔에 경고만 남기고 계속 진행**합니다 (로드를 막지 않습니다). 더 새 버전의 MCPTools가 저장한 문서일 수 있다는 뜻이므로, 결과가 이상하면 도구를 업데이트하세요.
- 로더는 모르는 키를 무시하므로, 구버전 MCPTools가 이 키가 든 문서를 읽어도 문제없습니다.
...
- 1단계 저장 시 이런 충돌을 검출해 안내합니다 — `mcptools_asset_list_save`는 `warnings`에 담아 반환하고, 1단계 창은 저장 직후 다이얼로그로 알립니다. **저장 자체는 막지 않습니다.**
...
- **브리지 요청이 403 "허용되지 않은 Host 헤더입니다"로 거부됨** — 브리지 서버는 브라우저를 통한 외부 페이지의 호출(DNS rebinding·교차 출처 요청)을 막기 위해, 루프백에 바인딩된 동안 요청의 `Host` 헤더가 `127.0.0.1` / `localhost` / `[::1]` 중 하나인지 확인합니다. 포트를 함께 적었다면 브리지가 실제로 열고 있는 포트와 같아야 합니다(포트 생략은 허용).
  - **조치** — `Tools/MCP/Settings`의 **브리지 서버 주소**를 `http://127.0.0.1:<포트>` 형식으로 맞추세요. 머신 이름이나 `http://내PC이름:8189` 같은 주소로 접속하면 거부됩니다.
...
- 현재 버전: **0.3.0** (패키지 이름 `com.sungchan.mcptools`)
