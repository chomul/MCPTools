...
namespace MCPTools.Editor
{
    ...
    /// - mcptools_asset_scan: 기획서 원문 + 프리팹 스캔 결과 + 항목 스키마/지침 반환 (AI가 분석해 목록 작성).
    /// - mcptools_asset_list_save: AI가 작성한 items 배열을 검증 후 목록 JSON으로 저장.
    /// </summary>
    [InitializeOnLoad]
    public static class AssetListupTool
    {
        static AssetListupTool()
        {
            McpToolRegistry.Register(
                ...
                "mcptools_asset_list_save에 넘길 수 있음).",
                ExecuteScan);

            McpToolRegistry.Register(
                "mcptools_asset_list_save",
                "AI가 작성한 에셋 목록(items 배열, mcptools_asset_scan의 itemSchema 형식)을 검증하고 " +
        ...
        private static object ExecuteScan(Dictionary<string, object> parameters)
        {
            ...
            if (GetBool(parameters, "scanOnly"))
            {
                ...
                data["items"] = itemDicts; // 그대로 mcptools_asset_list_save의 items로 전달 가능
                return data;
            }
        ...
        private static object ExecuteSave(Dictionary<string, object> parameters)
        {
                ...
                !(itemsObj is List<object> rawItems))
            {
                throw new System.ArgumentException(
                    "items 파라미터가 필요합니다 (mcptools_asset_scan의 itemSchema 형식 객체 배열).");
            }
