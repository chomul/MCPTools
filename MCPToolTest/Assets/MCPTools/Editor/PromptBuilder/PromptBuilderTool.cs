...
namespace MCPTools.Editor
{
    ...
    /// - mcptools_prompt_scan: 1단계 목록 항목 + 템플릿 + 프롬프트 스키마/지침 반환 (AI가 프롬프트 작성).
    /// - mcptools_prompt_save: AI가 작성한 items 배열을 검증 후 PromptSet JSON으로 저장.
    /// </summary>
    [InitializeOnLoad]
    public static class PromptBuilderTool
    {
        ...
        private static object ExecuteSave(Dictionary<string, object> parameters)
        {
            ...
            {
                throw new System.ArgumentException(
                    "items 파라미터가 필요합니다 (mcptools_prompt_scan의 promptSchema 형식 객체 배열).");
            }
