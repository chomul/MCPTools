// 2D Sprite 패키지(com.unity.2d.sprite) 의존 코드는 이 어셈블리(MCPTools.Editor.SpriteSlicing)에 격리한다.
// asmdef의 defineConstraints(MCPTOOLS_HAS_2D_SPRITE) 덕분에 패키지가 없는 프로젝트에서는
// 어셈블리 자체가 컴파일 대상에서 제외되어, 본체(MCPTools.Editor)는 정상 컴파일된다.
// 의존 방향은 SpriteSlicing → MCPTools.Editor 단방향이며, 본체는 훅(SpriteSheetImporter.SpriteRectWriter)으로만 호출한다.
using System.Collections.Generic;
using UnityEditor;
using UnityEditor.U2D.Sprites;
using UnityEngine;

namespace MCPTools.Editor.SpriteSlicing
{
    /// <summary>
