# AI Asset Pipeline — 기획서 기반 AI 에셋 생성 파이프라인

기획서(디자인 문서)를 입력으로 받아 AI로 게임 에셋(이미지/UI/사운드)을 생성하고, Unity 프리팹/UI에 자동 적용하는 **4단계 파이프라인**을 Unity Editor Tool로 제공합니다.

| 단계 | 메뉴 | 입력 → 출력 |
|------|------|------|
| 1. 에셋 리스트업 (AssetListup) | `Tools/AI Asset Pipeline/1. Asset Listup` | 기획서 + 프리팹·씬 스캔 → 생성할 에셋 목록 JSON (항목별 적용 대상 경로 포함) |
| 2. 프롬프트 제작 (PromptBuilder) | `Tools/AI Asset Pipeline/2. Prompt Builder` | 목록 JSON + 템플릿 → 항목별 positive/negative 프롬프트 JSON |
| 3. 생성 (ComfyUIGenerator) | `Tools/AI Asset Pipeline/3. ComfyUI Generator` | 프롬프트 + Workflow JSON → 항목별 후보 4개(1~12 조절) 생성·선택·확정 |
| 4. 적용 (AssetApplier) | `Tools/AI Asset Pipeline/4. Asset Applier` | 확정 결과물 → 프리팹/씬의 `Image`·`RawImage`·`SpriteRenderer`·`AudioSource`에 적용 (Undo 지원) |
| (별도) 스프라이트 시트 | `Tools/AI Asset Pipeline/Sprite Sheet` | 외부 AI로 만든 멀티 행 시트 png → 배경 제거·격자 검출·슬라이스 + 동작별 AnimationClip·AnimatorController |
| (통합) 파이프라인 | `Tools/AI Asset Pipeline/Pipeline (All-in-One)` | 1~4단계 진행 상황을 한 창에서 보고 중간 단계부터 재시작 |

파이프라인은 **AI 중립** 설계입니다. 분석/작성 지능은 사용자가 쓰는 AI(MCP 클라이언트, 로컬 AI CLI, 웹 AI)에 위임하고, 도구는 "재료 수집(scan)"과 "결과 검증·저장(save)"만 담당합니다.

> **MCP와의 관계** — 이 패키지는 **MCP 서버가 아닙니다.** 위 도구는 모두 Unity Editor 창만으로 동작합니다. 별도로 [MCP for Unity](https://github.com/CoplayDev/unity-mcp)(`com.coplaydev.unity-mcp`)를 설치한 프로젝트에 한해, 같은 기능이 `aiassetpipeline_*` **MCP 도구 16종으로 추가 노출**되어 AI 에이전트가 직접 호출할 수 있습니다. 해당 패키지가 없으면 연동 어셈블리가 컴파일에서 제외될 뿐 나머지 기능은 그대로입니다. → [MCP 도구 사용법](#mcp-도구-사용법)

## 빠른 시작 (첫 실행 체크리스트)

처음 도입할 때 이 순서대로 따라가면 첫 생성까지 도달합니다. 각 항목의 상세는 링크된 절을 참고하세요.

1. **(해당하는 경우) 기존 설치본 삭제** — 예전에 `Assets/` 아래에 `MCPTools`(v0.6.1 이하의 옛 이름) 또는 `AIAssetPipeline` 폴더를 직접 넣어 쓰던 프로젝트는 **패키지를 추가하기 전에** 그 폴더를 지웁니다. UPM으로 옛 `MCP Tools` 패키지를 설치해 둔 경우에도 먼저 제거하세요. → [설치](#설치)
2. **패키지 설치** — `Window > Package Manager`의 좌측 상단 **[+] > Add package from git URL**에 아래 URL을 입력합니다. 사용자 PC에 `git`이 설치되어 있어야 합니다. → [설치](#설치)
   ```
   https://github.com/chomul/Unity-AIAssetPipeline.git?path=MCPToolTest/Assets/AIAssetPipeline#v0.7.0
   ```
3. **컴파일·메뉴 확인** — 콘솔에 컴파일 오류가 없고 `Tools/AI Asset Pipeline/*` 메뉴가 보이는지 확인합니다.
...
6. **설정 확인** — `Tools/AI Asset Pipeline/Settings`를 열어 ComfyUI 주소를 확인합니다. 설정 에셋은 `Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset`에 자동 생성됩니다. → [AIAssetPipelineSettings 설정 항목](#aiassetpipelinesettings-설정-항목)
7. **브리지 서버 시작** — `Tools/AI Asset Pipeline/3. ComfyUI Generator` 상단의 **[서버 시작]** 을 누릅니다(3단계에서만 필요). ComfyUI 자체는 별도로 실행해야 합니다. → [3단계](#3단계--생성-comfyuigenerator-사용법)
8. **기획서 넣기** — `Assets/Docs` 폴더는 도구 창을 처음 열 때 자동 생성되지만, **기획서 `.md`/`.txt` 파일은 직접 넣어야** 합니다. → [요구 사항](#요구-사항)
9. **1단계 실행** — `Tools/AI Asset Pipeline/1. Asset Listup`으로 생성할 에셋 목록(`AssetList_*.json`)을 만듭니다. → [1단계](#1단계--에셋-리스트업-assetlistup-사용법)
10. **2 → 3 → 4단계** — 프롬프트 제작 → 후보 4개 생성·확정 → 프리팹/UI 적용 순으로 진행합니다. 전체 진행 상황은 `Tools/AI Asset Pipeline/Pipeline (All-in-One)`에서 한눈에 볼 수 있습니다. → [2단계](#2단계--프롬프트-제작-promptbuilder-사용법) · [3단계](#3단계--생성-comfyuigenerator-사용법) · [4단계](#4단계--적용-assetapplier-사용법)
...
- **검증 플랫폼: Windows 10/11. macOS·Linux는 미검증입니다.** 알려진 Windows 종속 지점은 [문제 해결](#문제-해결)의 마지막 항목에 정리해 두었습니다.
...
- **2D Sprite 패키지 (`com.unity.2d.sprite`) — 선택 사항.** 스프라이트 시트의 Sprite Mode=Multiple 슬라이스 적용에만 필요합니다. Unity 6에서 구 `TextureImporter.spritesheet` API가 동작하지 않아 모던 `ISpriteEditorDataProvider`(어셈블리 `Unity.2D.Sprite.Editor`)를 사용합니다. 대부분의 Unity 프로젝트 템플릿에 기본 포함되어 있습니다.
  - 패키지 의존 코드는 별도 어셈블리 `Editor/SpriteSlicing/`(`AIAssetPipeline.Editor.SpriteSlicing.asmdef`)에 격리되어 있고 **defineConstraints**(`AIASSETPIPELINE_HAS_2D_SPRITE`)가 걸려 있어, 패키지가 없으면 어셈블리 자체가 컴파일 대상에서 제외됩니다. 따라서 **미설치 시 스프라이트 시트 슬라이싱만 비활성화되고 나머지 기능(1~4단계 전 파이프라인)은 정상 동작**하며, 슬라이싱을 시도하면 설치 안내 메시지가 다이얼로그/도구 오류로 표시됩니다.
- **Python 3.7 이상** (3단계 생성용) — 브리지 서버(`Editor/ComfyUIGenerator/Server~/bridge_server.py`) 실행에 필요합니다. 표준 라이브러리만 사용하므로 pip 패키지 설치는 필요 없습니다.
  ...
  - 실행 파일은 [서버 시작] 시 **자동 탐지**됩니다(설정값 → `py -3`/`python`/`python3` → 표준 설치 폴더 → Anaconda/Miniconda 설치본 → PATH 순서로 찾아 실제 실행해 3.7 이상인지 검증). 자동 탐지에 실패하면 `Tools/AI Asset Pipeline/Settings`의 **Python 실행 파일**에 `python.exe` 절대 경로를 지정하거나 같은 창의 **[Python 자동 탐지]** 버튼을 누르세요.
...
- **unity-mcp (MCP For Unity, `com.coplaydev.unity-mcp`) 패키지 — 선택 사항.** MCP 도구(`aiassetpipeline_*`)를 MCP 클라이언트에 노출할 때만 필요합니다.
  - 패키지 의존 코드는 별도 어셈블리 `Editor/McpForUnityBridge/`(`AIAssetPipeline.Editor.McpForUnity.asmdef`)에 격리되어 있습니다. 이 asmdef는 **versionDefines**(`com.coplaydev.unity-mcp` 패키지가 존재하면 `AIASSETPIPELINE_HAS_MCPFORUNITY` 심볼 자동 정의)와 **defineConstraints**(`AIASSETPIPELINE_HAS_MCPFORUNITY` 요구)를 함께 사용하므로, 패키지가 없으면 어셈블리가 참조 해석 전에 통째로 제외되어 "non-existent assemblies" 컴파일 오류가 발생하지 않습니다. 패키지가 없는 프로젝트에서도 에디터 창만으로 전 기능을 사용할 수 있습니다.

## 설치

이 도구는 **GitHub + Unity Package Manager(UPM, git URL)** 로 배포합니다. zip·`.unitypackage` 배포는 하지 않습니다.

- 저장소: <https://github.com/chomul/Unity-AIAssetPipeline>
- 패키지 이름: `com.sungchan.aiassetpipeline` (현재 버전 `0.7.0`)

> ⚠️ **기존 설치본이 있다면 먼저 삭제하세요.** 예전에 `Assets/` 아래에 `AIAssetPipeline` 폴더를 직접 넣어 쓰던 프로젝트는 **패키지를 추가하기 전에 그 폴더를 반드시 삭제**해야 합니다. 남겨두면 어셈블리 이름이 중복되어 `Assembly with name 'AIAssetPipeline.Editor' already exists` 컴파일 오류가 나고, `.meta` GUID·설정 에셋도 두 벌이 됩니다.
> **`Assets/AIAssetPipeline.User/`는 사용자 데이터(설정 에셋·프롬프트 템플릿·워크플로 사본)이므로 지우면 안 됩니다.** 지울 대상은 도구 본체가 들어 있던 `AIAssetPipeline` 폴더뿐입니다.
...
4. 설치가 끝나면 컴파일 오류 없이 `Tools/AI Asset Pipeline/*` 메뉴가 나타납니다.
...
- URL 끝의 **`#v0.7.0`은 git 태그**입니다. 태그를 붙이면 해당 버전으로 **고정**되어 팀원 모두가 같은 버전을 받습니다.
- 태그를 **생략하면 기본 브랜치의 HEAD**를 받으므로, 설치 시점에 따라 서로 다른 버전이 될 수 있습니다. 팀 프로젝트에서는 태그를 붙이는 것을 권장합니다.
- 업데이트하려면 프로젝트의 `Packages/manifest.json`에서 이 패키지의 URL 태그를 새 버전으로 바꾸거나(예: `#v0.6.1` → `#v0.7.0`), Package Manager에서 **패키지를 제거한 뒤 새 URL로 다시 추가**합니다. 변경 이력은 [CHANGELOG.md](CHANGELOG.md)를 참고하세요.
- **v0.6.1 이하(`MCPTools`)에서 올라오는 경우**는 태그만 바꿔서는 안 됩니다. 패키지 이름·폴더·네임스페이스가 모두 바뀌었으므로 **기존 패키지를 제거한 뒤 새 URL로 다시 추가**하세요. 자세한 이관 절차는 [CHANGELOG.md](CHANGELOG.md)의 `0.7.0` 항목을 참고하세요.
- 패키지를 제거·재설치해도 `Assets/AIAssetPipeline.User/`의 설정·템플릿·워크플로 사본과 `Assets/Docs`·`Assets/Generated`의 산출물은 그대로 남습니다.

### 3) (선택) 로컬 개발용 설치

패키지를 직접 수정하며 개발하려면 폴더째 프로젝트의 **`Packages/` 아래에 넣어 embed**하거나(예: `Packages/com.sungchan.aiassetpipeline/`), `Assets/` 아래에 폴더째 복사해도 동작합니다. 설치 루트를 런타임에 계산하므로 세 형태(`Assets/`, `Packages/` embed, PackageCache) 모두에서 설정 로드·브리지 서버 시작·템플릿 탐색이 동작합니다. 단 **폴더 내부 구조는 그대로 유지**해야 합니다.

### 4) 첫 실행 시 생성되는 것

- `Tools/AI Asset Pipeline/*` 메뉴를 처음 사용하면 설정 에셋이 **`Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset`** 에 자동 생성됩니다. 패키지 폴더(`Packages/...`)는 읽기 전용이라 그 안에는 에셋을 만들 수 없기 때문입니다.
  - 설정 조회는 **`Assets` 범위만** 검색하므로, 기존에 `Assets/AIAssetPipeline/Editor/Common/AIAssetPipelineSettings.asset`을 쓰던 프로젝트는 **그 에셋을 계속 사용**합니다(새로 만들어지지 않음). `Assets` 아래에 설정 에셋이 2개 이상이면 첫 번째를 사용하고 콘솔에 중복 경로를 경고로 남깁니다.
...
- (선택) MCP 도구를 쓰려면 unity-mcp 패키지를 설치하고 MCP 클라이언트를 연결합니다. `Tools/AI Asset Pipeline/Ping (Local Test)` 메뉴 또는 `aiassetpipeline_ping` 호출로 연결을 확인할 수 있습니다.
...
### AIAssetPipelineSettings 설정 항목

설정 에셋 위치: **`Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset`** (자동 생성). 편집 창: **`Tools/AI Asset Pipeline/Settings`**.
...
| `spriteAnimationFrameRate` | 12 | 스프라이트 시트로 AnimationClip을 만들 때의 기본 프레임 레이트(fps). 키 시간은 `프레임 인덱스 / 이 값`으로 배치되며, 클수록 빨라집니다 (시트 창에서 항목별로 덮어쓸 수 있음) |
| `showBridgeConsole` | false | 브리지 서버 시작 시 콘솔 창 표시 여부. 끄면(기본) 창 없이 실행되고 로그는 `%TEMP%/aiassetpipeline_bridge_server.log`에 기록됩니다. 서버가 바로 죽는 원인을 볼 때 켜세요 |
...
- **VRAM이 부족해 연속 생성 중 OOM이 나는 환경에서만 켜세요.** `Tools/AI Asset Pipeline/Settings`의 "일괄 생성 후 모델 언로드" 체크박스, 또는 3단계 창의 같은 토글로 켤 수 있습니다.
...
설정 창(`Tools/AI Asset Pipeline/Settings`)의 버튼:
...
- **[워크플로를 프로젝트로 복사]** — 패키지에 동봉된 워크플로 JSON과 `variables.json`을 **`Assets/AIAssetPipeline.User/ComfyUI/`** 로 복사합니다. 복사본이 있으면 브리지 서버가 패키지 동봉본보다 **우선 사용**하므로, 읽기 전용인 UPM 설치에서도 워크플로/변수를 자유롭게 수정할 수 있습니다. 복사 후 브리지 서버를 재시작하세요. → [사용자 확장](#사용자-확장)
- **[서버 연결 테스트]** — 설정된 ComfyUI 주소로 연결을 확인합니다.

## 1단계 — 에셋 리스트업 (AssetListup) 사용법

메뉴: **`Tools/AI Asset Pipeline/1. Asset Listup`**
...
메뉴: **`Tools/AI Asset Pipeline/2. Prompt Builder`**
...
- **`Assets/AIAssetPipeline.User/Templates/<이름>.json`** — 사용자 템플릿. 패키지가 읽기 전용이어도 자유롭게 추가·수정할 수 있고, 이름이 겹치면 이쪽이 **우선**합니다. → [사용자 확장](#사용자-확장)
- `<AIAssetPipeline 설치 루트>/Editor/PromptBuilder/Templates/<이름>.json` — 패키지 동봉 기본값(읽기 전용).
...
메뉴: **`Tools/AI Asset Pipeline/3. ComfyUI Generator`**
...
워크플로 JSON은 `Server~/workflows/`에 **원본 구조 그대로** 있으며(토큰 치환 없음), 요청 시점에 노드 `inputs` 값만 덮어씁니다. 조정 가능한 변수는 `Server~/variables.json` 매니페스트로 정의됩니다. `Assets/AIAssetPipeline.User/ComfyUI/`에 사본이 있으면 브리지 서버가 **그쪽을 우선 사용**합니다 → [사용자 확장](#사용자-확장).
...
- Unity 에디터를 종료하면 **이 도구로 시작한 브리지 서버도 함께 종료**됩니다(설정 `shutdownBridgeOnEditorQuit`, 기본 켬). 외부에서 직접 실행한 서버나 다른 프로젝트가 띄운 서버는 건드리지 않습니다. 브리지를 계속 띄워두려면 `Tools/AI Asset Pipeline/Settings`의 **[종료 시 브리지 정리]** 를 끄세요.
...
2. **[후보 N개 생성]** — **후보 개수** 슬라이더(1~12, 기본 4)로 한 번에 생성할 개수를 조절합니다. 값은 설정 에셋의 `candidateCount`에 저장되어 `Tools/AI Asset Pipeline/Settings`와 공유되며, 전체 생성·MCP 도구 경로에도 같은 값이 적용됩니다. 개수가 많을수록 생성 시간과 VRAM 사용량이 늘어납니다.
...
항목을 클릭했을 때 **현재 워크플로가 이미 그 항목 종류에 해당하면 워크플로 선택을 유지**합니다(예: 이미지 항목을 `StyleChange`로 계속 작업). 종류가 다를 때만(오디오 항목을 이미지 워크플로에서 클릭하는 등) 기본 워크플로로 전환합니다.

### 4) 항목 추가·수정·삭제 (별도 편집 창)

2단계 창으로 돌아가지 않고 생성 창에서 항목을 손볼 수 있습니다. 항목 목록 아래 한 줄의 **[추가] [편집] [삭제] [저장]** 버튼을 사용합니다 (창 레이아웃은 기존 2열 그대로입니다).

- **[추가] / [편집]** — 별도의 **항목 편집 창**이 열립니다. ID(자동 부여, 읽기 전용), 이름, 종류(image/ui/audio), UI 여부, 대상 프리팹/오브젝트, 설명, positive/negative 프롬프트를 작성하고 **[저장]** 을 누르면 생성 창의 목록에 **추가(또는 같은 ID 항목 갱신)** 됩니다. [취소]하면 아무것도 바뀌지 않습니다.
  - **대상 프리팹**은 경로를 직접 입력하지 않고 **오브젝트 필드**(프로젝트 창에서 끌어다 놓거나 ◎ 버튼)로 고릅니다. **대상 오브젝트**는 그 프리팹의 계층을 읽어 만든 **드롭다운**에서 선택하며, 적용 가능한 컴포넌트가 있는 오브젝트에는 `[Image]` `[RawImage]` `[SpriteRenderer]` `[AudioSource]` 표시가 붙습니다. 프리팹을 바꾸면 대상 오브젝트 선택은 초기화됩니다. 저장된 경로의 프리팹/오브젝트를 찾을 수 없으면(삭제·이동·계층 변경) 값을 지우지 않고 경고로 표시합니다.
  - 새 항목은 현재 워크플로 종류(audio/ui/image)에 맞춰 만들어져 목록에 바로 나타납니다. **프롬프트는 빈칸으로 시작**하며, 생성 창 변수란에 써 둔 값이 필요하면 **[생성 창의 현재 프롬프트 가져오기]** 버튼으로 가져옵니다.
  - 목록에 반영된 항목은 **즉시 생성에 사용**할 수 있습니다(프롬프트는 오른쪽 변수 편집란에도 동기화됩니다).
- **[삭제]** — 선택 항목을 목록에서만 제거합니다. **이미 생성된 후보/확정 파일은 지우지 않습니다.**
- **[저장]** — 목록 전체를 PromptSet JSON에 기록합니다. 로드한 파일이 있으면 그 파일에 덮어쓰고, 없으면 `Docs/2_PromptSet/PromptSet_{yyyyMMdd_HHmm}.json`으로 새로 저장합니다. 저장하지 않은 변경이 있으면 버튼이 **[저장 *]** 로 표시되고, 다른 PromptSet을 로드할 때 확인 다이얼로그가 뜹니다.
- PromptSet을 로드하지 않은 상태에서도 입력 영역의 **[항목 추가]** 로 빈 목록을 시작할 수 있습니다.

## 4단계 — 적용 (AssetApplier) 사용법

메뉴: **Tools/AI Asset Pipeline/4. Asset Applier**. 3단계에서 확정한 결과물을 AssetList에 기록된 대상 프리팹 또는 씬(`targetScenePath` 항목)의 컴포넌트에 적용합니다. 프리팹 항목은 씬 인스턴스가 아닌 **프리팹 에셋 자체**를 수정하며, 적용 후 Ctrl+Z로 되돌릴 수 있습니다. 씬 항목은 씬에 직접 배치된 오브젝트에 적용합니다 — 씬이 이미 열려 있으면 그대로 값만 변경(dirty 표시, Ctrl+Z 가능, 저장은 사용자 몫), 열려 있지 않으면 Additive로 열어 적용·저장 후 닫습니다. 일괄 적용 시 같은 씬 항목은 묶어서 씬을 한 번만 엽니다.

1. **AssetList JSON** 드롭다운(최신 파일 기본)에서 1단계 산출물을 [로드]합니다.
2. 항목 목록에 상태 배지가 표시됩니다 — `확정본 없음`(3단계 미확정) / `검증 실패`(사유는 항목 선택 시 표시) / `적용 준비` / `적용됨`.
   - **`적용됨` 상태는 창을 닫았다 다시 열어도 유지됩니다.** 적용에 성공하면 `Assets/Generated/3_Confirmed/GenerationResults.json`의 `applications` 배열에 항목 id·대상·적용한 에셋 경로·적용 시각이 기록되고, 목록을 로드할 때 이 기록에서 복원합니다. 1단계 산출물(AssetList JSON)은 건드리지 않습니다.
...
4. **[선택 적용]** 또는 **[일괄 적용(검증 통과 항목 전체)]** — 적용 후 성공/실패 요약(실패 사유 포함)이 표시되고 `AssetDatabase.SaveAssets`로 마무리됩니다.
   - **[일괄 적용]은 이미 적용한 항목을 건너뜁니다.** 프리팹을 불필요하게 다시 저장해 git diff가 생기는 것을 막기 위해서입니다. 건너뛴 개수는 버튼 아래 "이미 적용됨 N개"로 표시됩니다.
...
- **변종 프리팹**(변종의 변종 포함): 베이스에서 상속한 컴포넌트에 적용하면 **그 변종 에셋에만 오버라이드로 기록**됩니다. 베이스와 중간 변종은 변하지 않습니다.
- **중첩 프리팹**: 바깥 프리팹 안의 중첩 인스턴스에 적용하면 **바깥 프리팹에 오버라이드로 기록**되고, 중첩된 원본 프리팹은 변하지 않습니다.
...
메뉴: **`Tools/AI Asset Pipeline/Sprite Sheet`**. ComfyUI가 아니라 **외부 AI(ChatGPT 이미지 생성 등)** 로 만든 멀티 행 시트를 다루는 별도 도구입니다. 한 장의 시트에 동작(walk/run/attack/…)을 행별로 담고, 그것을 잘라 Unity 스프라이트와 AnimationClip으로 만듭니다.

> 슬라이스에는 **2D Sprite 패키지**가 필요합니다([요구 사항](#요구-사항) 참조).

### 1) 프롬프트 생성 → 외부 AI

동작 행 목록(동작명 + 프레임 수), 게임 장르·아트 스타일, 셀 크기·방향·배경을 입력하면 검증된 구조의 프롬프트를 만듭니다. AI CLI가 있으면 그것으로 다듬고, 없으면 템플릿으로 생성합니다.

**바라보는 방향**은 오른쪽(RIGHT) / 왼쪽(LEFT) / **정면(FRONT)** 중에서 고릅니다. 오른쪽·왼쪽은 측면(side-scrolling / side-view) 시트로, 정면은 카메라를 바라보는 **front-view** 시트로 프롬프트가 조립됩니다 — 시점 문구와 걷기·달리기 연출 지시("side-walk" → "walking toward the viewer")가 함께 바뀝니다. **[클립보드 복사]** 후 레퍼런스 이미지와 함께 외부 AI에 붙여넣어 시트를 생성하세요. 답변은 `Assets/Docs/SpriteSheetPrompt/`에 저장됩니다.
...
3. **[배경 제거 + 격자 검출]** — 흰 배경을 외곽 flood-fill로 제거하고 AI가 그린 격자선을 검출합니다. **이 단계에서는 아무것도 기록하지 않습니다.**
   - flood-fill은 이미지 네 외곽에서 시작해 ① 이웃과의 색 차가 허용 오차 이내이거나, ② 근사 흰색이거나, ③ **밝은 무채색이면서 지금 픽셀보다 밝은** 픽셀로 번집니다. ③은 시트 가장자리 격자선(회색)과 흰 배경 사이의 안티에일리어싱 계단(예: 159 → 216 → 252)을 넘기 위한 조건입니다.
   - 여기에 더해 ④ **얇은 무채색 장벽 건너뛰기** — 한 방향으로 최대 4px까지 무채색만 지나 근사 흰색 배경에 닿으면 그 사이를 함께 배경으로 편입합니다. AI가 그린 격자선은 근사 흰색보다 훨씬 어두울 수 있어(실측 min 채널 133~180) ①~③으로는 넘을 수 없고, 넘지 못하면 **외곽에서 닿는 셀만 지워지고 나머지 셀의 흰 배경이 남습니다.** 4px보다 두꺼운 무채색 덩어리(회색조 캐릭터 몸통 등)는 근사 흰색에 닿지 못해 통과하지 못합니다.
   - 유채색 픽셀은 어떤 경로로도 배경이 되지 않아 글로우 이펙트가 보존됩니다. 좁은 틈으로 캐릭터 내부에 새어 든 영역은 pocket restore로 복원되며, 위 ④로 건너뛴 격자선 픽셀은 연결자로 취급해 셀 배경이 "고립된 pocket"으로 오판되지 않게 합니다.
...
메뉴: **`Tools/AI Asset Pipeline/Pipeline (All-in-One)`**
...
MCP 도구로 후반부를 자동화하려면 아래 `aiassetpipeline_run_pipeline`을, 현황 진단은 `aiassetpipeline_status`를 사용하세요.

## MCP 도구 사용법

**이 절은 [MCP for Unity](https://github.com/CoplayDev/unity-mcp)(`com.coplaydev.unity-mcp`)를 함께 설치한 경우에만 해당합니다.** 이 패키지 자체는 MCP 서버가 아니며, 아래 도구는 그 패키지의 브리지에 등록되어 노출됩니다. 설치되어 있지 않으면 연동 어셈블리(`AIAssetPipeline.Editor.McpForUnity`)가 컴파일에서 제외되고 도구도 나타나지 않습니다 — Editor 창 기능은 영향받지 않습니다. 등록된 도구 목록은 `Tools/AI Asset Pipeline/Settings` 창 하단에서도 확인할 수 있습니다.

모든 도구의 반환 공통 포맷: `{ "success": bool, "message": string, "data": {...} }`. 실패 시 `success:false` + `message`에 한국어 원인 안내. 경로는 전부 `Assets/` 기준 상대 경로 문자열입니다.

**Play Mode·컴파일 중에는 도구가 실행되지 않습니다.** 에디터가 재생 중이거나 스크립트 컴파일/에셋 임포트 중이면 도구를 실행하지 않고 `success:false` + 사유("에디터가 Play Mode입니다. 재생을 멈춘 뒤 다시 호출해주세요.")를 반환합니다. 그 상태에서 `AssetDatabase.Refresh()`가 도메인 리로드를 유발하면 진행 중인 생성 작업과 플레이 세션이 함께 끊기기 때문입니다.
예외로 **읽기 전용 진단 도구 3개**(`aiassetpipeline_ping`, `aiassetpipeline_status`, `aiassetpipeline_list_candidates`)는 언제든 호출할 수 있습니다. `list_candidates`는 생성 Job의 완료를 확인하는 폴링 경로라 막지 않습니다.
같은 조건에서 각 도구 창의 생성·확정·적용·스캔·저장 버튼도 비활성화되고 창 상단에 사유가 표시됩니다.

**모달 다이얼로그 없음**: MCP 경로에서 실행되는 코드는 확인 창을 띄우지 않습니다. 실패는 전부 `message`(또는 생성 Job의 `failed` 상태)로 전달되므로, 에이전트가 호출한 뒤 사람이 [확인]을 누를 때까지 에디터가 잠기는 일이 없습니다.

### `aiassetpipeline_ping` — 연결 진단

- 파라미터: 없음
- 반환 `data`: `{ version, unityVersion, serverUrl }`

### `aiassetpipeline_asset_scan` — 1단계 분석 입력 수집
...
- 파라미터:
  ...
  - `scanOnly: bool` (선택, 기본 `false`) — `true`면 기획서에서 항목을 추출하지 않고 **현재 열려 있는 씬의 슬롯 + `scanRootPath` 아래 모든 프리팹의 슬롯**을 병합해 스캔합니다 (`scenePaths`는 무시). 씬에 포함된 프리팹과 스캔 루트 프리팹이 겹치면 프리팹 단위로 한 번만 담기며, 열린 씬이 비어 있어도 프리팹 스캔 결과로 목록이 만들어집니다. 반환 `data`에 대상 경로가 채워진 완성 `items` 배열이 포함되어 그대로 `aiassetpipeline_asset_list_save`에 전달할 수 있습니다. `designDocPath`는 이때도 그대로 읽혀 `designDocPath`/`designDocText`로 반환되고 items 문서의 컨텍스트로 기록되니, 저장 시 `aiassetpipeline_asset_list_save`의 `designDocPath`에도 함께 넘겨 2단계에서 참고하게 하세요.
- 반환 `data`:
...
### `aiassetpipeline_asset_list_save` — 1단계 목록 검증·저장
...
### `aiassetpipeline_prompt_scan` — 2단계 프롬프트 재료 수집
...
### `aiassetpipeline_prompt_save` — 2단계 프롬프트 검증·저장
...
### `aiassetpipeline_generate_candidates` — 3단계 후보 생성 Job 시작

생성은 수십 초~수 분이 걸리므로 **Job 방식**입니다: 이 도구는 Job을 시작하고 즉시 반환하며, 완료 여부는 `aiassetpipeline_list_candidates`로 폴링합니다.
...
- 파라미터:
  ...
  - `scope: string` (선택) — **PromptSet 단위 저장 스코프** (보통 PromptSet 파일명에서 확장자를 뗀 값, 예: `PromptSet_20260721_0512`). 지정하면 후보가 `3_Candidates/{scope}/{항목id}/`에 저장되어 다른 PromptSet의 같은 id와 충돌하지 않습니다. 이후 `aiassetpipeline_list_candidates`/`aiassetpipeline_select_candidate`에도 **같은 값**을 넘기세요. **생략 시 스코프 없이 기존 위치를 사용합니다 (기존 호출과 호환).**
- 반환 `data`: `{ status: "started", assetItemId, scope, candidateFolder }`
- 같은 항목(같은 스코프)의 Job이 이미 실행 중이면 오류를 반환합니다. 기존 후보는 삭제 후 새로 생성됩니다.

### `aiassetpipeline_list_candidates` — 후보 목록/Job 상태 조회
...
### `aiassetpipeline_select_candidate` — 후보 확정
...
### `aiassetpipeline_delete_candidate` — 후보 삭제

- 파라미터: `assetItemId: string` (필수), `candidatePath: string` (필수, `aiassetpipeline_list_candidates`가 반환한 후보 경로 — `Assets/` 기준 상대 경로)
...
### `aiassetpipeline_apply_asset` — 4단계 단건 적용
...
### `aiassetpipeline_apply_all` — 4단계 일괄 적용
...
### `aiassetpipeline_run_pipeline` — 후반부(3~4단계) 자동화
...
- **진행 상황과 최종 결과는 `aiassetpipeline_status`의 `pipeline` 블록으로 폴링**합니다.
  - `pendingSelections` — `autoSelect="none"`에서 채워짐: `[{ assetItemId, candidates: [{ path, seed }] }]`. 이후 `aiassetpipeline_select_candidate` + `aiassetpipeline_apply_asset`으로 진행.
...
### `aiassetpipeline_spritesheet_build_prompt` — 시트 프롬프트 조립

- 파라미터: `rows`(선택, `"walk:8,run:8,attack:8,death:10"` 형식), `useReferenceImage`(선택, 기본 true), `characterDescription`(선택 — `useReferenceImage=false`면 필수), `genre`·`artStyle`·`notes`(선택, 영어 자유 텍스트), `cellSize`(기본 256), `direction`(right/left/front — front는 front-view 문구로 조립), `background`(white/transparent)
- 반환: 프롬프트 문자열 + 저장 경로. 반환한 프롬프트를 레퍼런스 이미지와 함께 외부 AI에 붙여넣어 시트를 만듭니다.

### `aiassetpipeline_spritesheet_import` — 시트 슬라이스
...
### `aiassetpipeline_spritesheet_build_clips` — 동작별 AnimationClip 생성
...
- 반환 `data`:
  ...
  - `pipeline` — `aiassetpipeline_run_pipeline` Job의 진행 상태: `{ status: "idle"|"running"|"completed"|"failed", message, phase, promptSetPath, assetListPath, autoSelect, itemCount, currentIndex, currentItemId, currentItemProgress, elapsedSeconds, timeoutSeconds, pendingSelections, applied, failed }`. **run_pipeline의 완료 여부와 결과를 이 블록으로 폴링합니다.**
...
| 필드 | 설명 |
|------|------|
| `id` | 항목 고유 ID (`item_001` 형식, 생략 시 자동 부여). 3단계에서 **후보 폴더명·확정본 파일명이 되므로** 파일명에 쓸 수 없는 문자(`" < > \| : * ? \ /`)는 `_`로 바뀝니다 — 아래 "id 충돌" 참조 |
| `name` | 에셋 이름 (필수) |
...
### `schemaVersion` — 문서 스키마 버전

AssetList / PromptSet / GenerationResults 세 문서의 최상위에 정수 `schemaVersion`이 붙습니다 (현재 **1**).

- 이 키가 **없는 구버전 파일은 버전 1로 간주**해 그대로 읽습니다. 손으로 쓴 문서도 마찬가지입니다.
- 읽은 값이 도구가 아는 버전보다 **크면 콘솔에 경고만 남기고 계속 진행**합니다 (로드를 막지 않습니다). 더 새 버전의 AIAssetPipeline가 저장한 문서일 수 있다는 뜻이므로, 결과가 이상하면 도구를 업데이트하세요.
- 로더는 모르는 키를 무시하므로, 구버전 AIAssetPipeline가 이 키가 든 문서를 읽어도 문제없습니다.
- 외부 AI에게 요청하는 것은 **항목 배열**이지 문서 전체가 아니므로, AI 응답에 `schemaVersion`을 넣을 필요가 없습니다. 저장할 때 도구가 붙입니다.

### id 충돌 — 같은 파일명이 되는 항목 id

서로 다른 항목 id가 파일명 변환 후 같아지면(예: `hero:idle`과 `hero/idle`이 둘 다 `hero_idle`) 3단계 후보 폴더와 확정본이 **서로 덮어써집니다.**

- 1단계 저장 시 이런 충돌을 검출해 안내합니다 — `aiassetpipeline_asset_list_save`는 `warnings`에 담아 반환하고, 1단계 창은 저장 직후 다이얼로그로 알립니다. **저장 자체는 막지 않습니다.**
- 3단계에서 다른 항목의 후보 폴더를 지우거나 다른 항목의 확정본을 덮어쓰게 되면 콘솔 경고를 남깁니다(같은 항목의 재생성·재확정은 정상 동작이므로 경고하지 않습니다).
- 조치: 항목 id를 파일명에 쓸 수 있는 문자(영문·숫자·`_`·`-`)로 구분하세요.

### 적용 이력 — `GenerationResults.json`의 `applications`

`Assets/Generated/3_Confirmed/GenerationResults.json` 한 문서에 3단계 확정 기록(`results`)과 4단계 적용 이력(`applications`)이 형제 키로 함께 들어갑니다.
  ...
  "applications": [
    {
      ...
      "targetScenePath": "",
      "targetObjectPath": "Canvas/TitlePanel/Logo",
      "appliedAssetPath": "Assets/Generated/3_Confirmed/Images/item_001.png",
      "spriteName": "",
      "linkedControllerPath": "",
      "appliedAt": "2026-07-26 01:15:20"
    }
  ]
}
```
...
- `AIAssetPipelineToolRegistry` — MCP 도구 이름→핸들러 레지스트리. unity-mcp 브리지 의존은 별도 어셈블리 `Editor/McpForUnityBridge/`에 격리.
...
UPM으로 설치한 패키지 폴더(`Packages/...`)는 **읽기 전용**입니다. 그래서 사용자가 추가·수정하는 것들은 모두 프로젝트 쪽 **`Assets/AIAssetPipeline.User/`** 아래에 둡니다.

| 용도 | 위치 | 만드는 방법 |
|------|------|-------------|
| 설정 에셋 | `Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset` | `Tools/AI Asset Pipeline/*` 첫 사용 시 자동 생성 |
| 프롬프트 템플릿 | `Assets/AIAssetPipeline.User/Templates/<이름>.json` | 폴더를 만들고 JSON 파일을 직접 추가 |
| 워크플로 JSON·`variables.json` 사본 | `Assets/AIAssetPipeline.User/ComfyUI/workflows/*.json`, `Assets/AIAssetPipeline.User/ComfyUI/variables.json` | 설정 창의 **[워크플로를 프로젝트로 복사]** 버튼 |
...
1. 프로젝트에 `Assets/AIAssetPipeline.User/Templates/` 폴더를 만듭니다.
...
- **워크플로 구조를 바꾸거나 워크플로를 추가하는 경우** — 설정 창(`Tools/AI Asset Pipeline/Settings`)의 **[워크플로를 프로젝트로 복사]** 를 눌러 패키지 동봉본을 `Assets/AIAssetPipeline.User/ComfyUI/`로 복사한 뒤, 그 사본의 JSON을 편집합니다.
  - 브리지 서버는 `Assets/AIAssetPipeline.User/ComfyUI/workflows/<이름>.json`이 있으면 **패키지 동봉본보다 우선** 로드하고, 사본에만 있는 이름은 워크플로 목록에 추가로 나타납니다.
...
- **연속 생성이 회차마다 느려짐 (회차당 10~40초 추가)** — `unloadModelsAfterBatch`가 켜져 있으면 생성이 끝날 때마다 ComfyUI 모델이 언로드되어 다음 생성에서 체크포인트를 다시 로드합니다. 기본값은 꺼짐이며, `Tools/AI Asset Pipeline/Settings`에서 확인하세요. → [VRAM 트레이드오프](#aiassetpipelinesettings-설정-항목)
...
- **`Assembly with name 'AIAssetPipeline.Editor' already exists` 컴파일 오류** — `Assets/` 아래에 예전 설치본(`AIAssetPipeline` 폴더)이 남은 채 패키지를 추가한 경우입니다. **`Assets/` 쪽 `AIAssetPipeline` 폴더를 삭제**하세요. `Assets/AIAssetPipeline.User/`는 사용자 데이터이므로 **지우면 안 됩니다.** → [설치](#설치)
- **설정이 저장되지 않거나 값이 되돌아감** — 설정 에셋이 여러 개인지 확인하세요. 콘솔에 "설정 에셋이 N개 발견되어 첫 번째를 사용합니다" 경고와 경로 목록이 출력됩니다. 사용하지 않는 에셋을 지우면 됩니다. 정상 위치는 `Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset`입니다(구버전 설치본의 `Assets/AIAssetPipeline/Editor/Common/AIAssetPipelineSettings.asset`도 그대로 사용됨).
- **패키지를 업데이트했더니 직접 고친 워크플로/템플릿이 사라짐** — 패키지 폴더는 재해결 시 원본으로 되돌아갑니다. 수정본은 `Assets/AIAssetPipeline.User/` 아래에 두세요. → [사용자 확장](#사용자-확장)
- **MCP 도구가 목록에 안 보임** — unity-mcp 패키지 설치 여부와 `AIASSETPIPELINE_HAS_MCPFORUNITY` 심볼 정의(패키지 설치 시 자동)를 확인하고, 파라미터 스키마 변경 후에는 MCP 서버를 재시작하세요.
...
- **브리지 서버가 시작되지 않을 때 ([서버 시작] 실패)** — 아래를 순서대로 확인하세요.
  ...
  6. **직접 지정 / 자동 탐지** — `Tools/AI Asset Pipeline/Settings`의 **Python 실행 파일**에 `python.exe` 절대 경로를 입력하거나, 같은 창의 **[Python 자동 탐지]** 버튼을 눌러 자동으로 채우세요(찾은 경로와 버전이 다이얼로그로 표시됩니다).
  7. **로그 확인** — "브리지 콘솔 창 표시"가 꺼져 있으면 서버 로그는 `%TEMP%/aiassetpipeline_bridge_server.log`(시스템 임시 폴더)에 기록됩니다. 켜두면 콘솔 창에서 오류를 바로 볼 수 있습니다.
...
- **브리지 요청이 403 "허용되지 않은 Host 헤더입니다"로 거부됨** — 브리지 서버는 브라우저를 통한 외부 페이지의 호출(DNS rebinding·교차 출처 요청)을 막기 위해, 루프백에 바인딩된 동안 요청의 `Host` 헤더가 `127.0.0.1` / `localhost` / `[::1]` 중 하나인지 확인합니다. 포트를 함께 적었다면 브리지가 실제로 열고 있는 포트와 같아야 합니다(포트 생략은 허용).
  - **조치** — `Tools/AI Asset Pipeline/Settings`의 **브리지 서버 주소**를 `http://127.0.0.1:<포트>` 형식으로 맞추세요. 머신 이름이나 `http://내PC이름:8189` 같은 주소로 접속하면 거부됩니다.
  - **정책: `--host`로 외부 바인딩한 경우 Host 검증은 비활성화됩니다.** 루프백이 아닌 주소(`0.0.0.0`, 사설 IP 등)로 띄우는 것은 "다른 기기에서 접속시키겠다"는 의도적 노출이고, 그때 쓰일 호스트명을 서버가 알 수 없어 허용 목록을 만들 수 없기 때문입니다. 이 경우 기동 로그에 `외부 바인딩이므로 Host 검증이 비활성화됩니다. 신뢰할 수 있는 네트워크에서만 사용하세요.` 경고가 출력됩니다. **신뢰할 수 있는 네트워크에서만 사용하세요.** (같은 이유로 외부 바인딩 시 `POST /shutdown`은 항상 403으로 거부됩니다.)
- **브리지 요청이 400으로 거부됨 (입력 상한)** — 오타 하나로 수천 건이 ComfyUI 큐에 들어가는 것을 막기 위해 브리지가 입력에 상한을 둡니다. 조용히 보정하지 않고 사유를 담아 거절하므로, 메시지를 보고 요청 값을 고치세요.
  - **후보 개수 `count`는 1~32** — 범위를 벗어나면 `count는 1~32 사이여야 합니다 (요청값: 9999).` 처럼 거부됩니다. 값이 없거나 `0`이면 기본값 4로 처리합니다.
...
- 저장소·이슈: <https://github.com/chomul/Unity-AIAssetPipeline>
