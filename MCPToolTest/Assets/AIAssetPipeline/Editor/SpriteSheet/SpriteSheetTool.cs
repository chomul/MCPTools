...
namespace AIAssetPipeline.Editor
{
    ...
    /// - aiassetpipeline_spritesheet_build_prompt: 멀티 행(동작별 Row) 통합 시트 프롬프트 조립 + JSON 저장.
    /// - aiassetpipeline_spritesheet_import: 외부 AI 멀티 행 시트 png → 배경 제거·정규화 + 동작명 기반 Sprite Multiple 슬라이스.
    /// - aiassetpipeline_spritesheet_build_clips: 슬라이스된 시트 → 동작별 AnimationClip(+ AnimatorController·프리팹 연결).
    /// </summary>
    [InitializeOnLoad]
    public static class SpriteSheetTool
    {
        static SpriteSheetTool()
        {
            AIAssetPipelineToolRegistry.Register(
                "aiassetpipeline_spritesheet_build_prompt",
            ...
            AIAssetPipelineToolRegistry.Register(
                "aiassetpipeline_spritesheet_import",
            ...
            AIAssetPipelineToolRegistry.Register(
                "aiassetpipeline_spritesheet_build_clips",
