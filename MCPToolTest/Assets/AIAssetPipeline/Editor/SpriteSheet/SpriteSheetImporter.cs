...
namespace AIAssetPipeline.Editor
{
    ...
    public static class SpriteSheetImporter
    {
        ...
        /// <c>AIAssetPipeline.Editor.SpriteSlicing</c>에 있으며, 패키지가 설치된 프로젝트에서만
        ...
        {
            string folder = AIAssetPipelineFolders.SpriteSheetsDir(AIAssetPipelineSettings.GetOrCreate());
        ...
        private static string SaveSheet(Color32[] sheet, int width, int height, string baseName)
        {
            ...
            string folder = AIAssetPipelineFolders.SpriteSheetsDir(settings);
        ...
        private static void ApplySpriteSlices(string assetPath, List<SpriteMetaData> metas)
        {
            ...
            // 해당 API는 2D Sprite 패키지 소속이라 선택적 어셈블리(AIAssetPipeline.Editor.SpriteSlicing)로 분리했고,
            // 여기서는 등록된 훅으로만 호출한다.
