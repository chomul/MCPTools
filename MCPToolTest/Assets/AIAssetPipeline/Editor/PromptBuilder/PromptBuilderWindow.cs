...
{
    /// <summary>
    /// 2단계 프롬프트 제작 창입니다. (메뉴: Tools/AI Asset Pipeline/2. Prompt Builder)
    ...
    public class PromptBuilderWindow : EditorWindow
    {
        ...
        private const string PrefKeySelectedAi = "AIAssetPipeline.PromptBuilder.SelectedAiTool";
        private const string PrefKeyCustomCommand = "AIAssetPipeline.PromptBuilder.CustomAiCommand";
        private const string PrefKeyTimeout = "AIAssetPipeline.PromptBuilder.AiTimeoutSeconds";
        private const string PrefKeyAllowExplore = "AIAssetPipeline.PromptBuilder.AiAllowProjectExplore";
        private const string PrefKeyManualFoldout = "AIAssetPipeline.PromptBuilder.ManualFoldout";
        ...
        [MenuItem("Tools/AI Asset Pipeline/2. Prompt Builder", false, 2)]
        ...
        private void OnEnable()
        {
            ...
            AIAssetPipelineFolders.EnsureWorkFolders(_settings);
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    if (_assetListPaths.Length == 0)
                    {
                        EditorGUILayout.HelpBox(
                            ...
                            "1단계(Tools/AI Asset Pipeline/1. Asset Listup)에서 목록을 먼저 저장해주세요.", MessageType.Warning);
                    }
                    else
                    {
        ...
        private AssetListDocument LoadSelectedAssetList(out string listPath)
        {
            ...
            if (string.IsNullOrEmpty(listPath))
            {
                EditorUtility.DisplayDialog("프롬프트 빌더",
                    $"에셋 목록 JSON이 없습니다.\n{AIAssetPipelineFolders.AssetListDir(_settings)} 폴더에 1단계 산출물(AssetList_*.json)을 먼저 저장해주세요.",
                    "확인");
                return null;
            }
        ...
        private void Import(bool merge)
        {
            if (_owner == null)
            {
                EditorUtility.DisplayDialog("AI 응답 불러오기",
                    "프롬프트 빌더 창을 찾을 수 없습니다. Tools/AI Asset Pipeline/2. Prompt Builder 창을 연 뒤 다시 시도해주세요.", "확인");
                return;
            }
