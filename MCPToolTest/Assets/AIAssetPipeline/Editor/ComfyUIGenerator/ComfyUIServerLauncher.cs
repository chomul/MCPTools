...
namespace AIAssetPipeline.Editor
{
    ...
    /// Python 실행 파일은 <see cref="AIAssetPipelineSettings.pythonExecutable"/>을 우선 사용하되,
    ...
    {
        private const string SessionKeyPid = "AIAssetPipeline.Bridge.ServerPid";
        private const string SessionKeyPythonPath = "AIAssetPipeline.Bridge.PythonPath";
        private const string SessionKeyPythonArgs = "AIAssetPipeline.Bridge.PythonArgs";
        private const string SessionKeyPythonVersion = "AIAssetPipeline.Bridge.PythonVersion";
        private const string SessionKeyPythonSetting = "AIAssetPipeline.Bridge.PythonSetting";
        private const string SessionKeyIdentityChecked = "AIAssetPipeline.Bridge.IdentityChecked";
        ...
        /// <summary>
        /// 사용자 워크플로/변수 오버라이드 폴더입니다 (에셋 경로 기준, 고정: "Assets/AIAssetPipeline.User/ComfyUI").
        ...
        /// </summary>
        public const string UserComfyDirAssetPath = "Assets/AIAssetPipeline.User/ComfyUI";
        ...
        {
            string root = AIAssetPipelineSettings.InstallRoot.Replace('\\', '/').TrimEnd('/');
            ...
            if (packageInfo != null)
            {
                ...
                // 설치 루트가 패키지 루트보다 깊으면 (예: "Packages/<패키지명>/AIAssetPipeline") 그 하위 경로를 보존한다.
        ...
        {
            string root = AIAssetPipelineSettings.InstallRoot;
            ...
            message.AppendLine($"인식된 AIAssetPipeline 설치 루트: \"{root}\"");
            ...
            {
                message.AppendLine("2. Assets 설치라면 AIAssetPipeline 폴더를 옮기거나 복사할 때 Server~ 폴더가 함께 복사됐는지 확인해주세요 " +
        ...
        public static async Task<bool> WarnIfForeignBridgeAsync()
        {
            ...
            UnityEngine.Debug.LogWarning(
                $"[AIAssetPipeline] 다른 위치의 브리지 서버가 실행 중입니다 " +
                $"(실행 중: \"{runningScriptPath}\"{versionNote} / 이 프로젝트: \"{ownScriptPath}\"). " +
                "워크플로·변수가 이 프로젝트의 것과 다를 수 있습니다. " +
                $"다른 Unity 프로젝트의 브리지를 종료하거나, Tools/AI Asset Pipeline/Settings에서 브리지 서버 주소({bridgeUrl})의 " +
        ...
        {
            string[] guids = AssetDatabase.FindAssets("t:AIAssetPipelineSettings", new[] { "Assets" });
            if (guids == null || guids.Length == 0)
            {
                return string.Empty;
            }

            var settings = AssetDatabase.LoadAssetAtPath<AIAssetPipelineSettings>(AssetDatabase.GUIDToAssetPath(guids[0]));
        ...
        public static string ResolvePython(AIAssetPipelineSettings settings, out string argumentPrefix, out string version, out string diagnostics)
        {
        ...
        public static string BuildPythonNotFoundMessage(string diagnostics)
        {
            ...
            message.AppendLine("5. 그래도 안 되면 Tools/AI Asset Pipeline/Settings의 \"Python 실행 파일\"에 python.exe의 절대 경로를 직접 입력하세요 " +
        ...
        public static void Start(AIAssetPipelineSettings settings)
        {
            if (settings == null)
            {
            ...
            if (!settings.showBridgeConsole)
            {
                ...
                logPath = Path.Combine(Path.GetTempPath(), "aiassetpipeline_bridge_server.log").Replace('\\', '/');
            ...
            catch (Exception e)
            {
                ...
                throw new InvalidOperationException(
                    ...
                    $"실행 대상: \"{python}\"\n" +
                    "Tools/AI Asset Pipeline/Settings에서 [자동 탐지] 버튼을 누르거나 python.exe의 절대 경로를 직접 지정해주세요." +
                    longPathNote, e);
            }
            ...
            UnityEngine.Debug.Log(string.IsNullOrEmpty(logPath)
                ? $"[AIAssetPipeline] 브리지 서버를 시작했습니다 (PID {process.Id}, 포트 {port}{versionNote}, 실행 파일 {python})."
                : $"[AIAssetPipeline] 브리지 서버를 시작했습니다 (PID {process.Id}, 포트 {port}{versionNote}, 실행 파일 {python}, 로그: {logPath}).");
        }
        ...
        private static void OnEditorQuitting()
        {
            ...
            try
            {
                if (!AIAssetPipelineSettings.GetOrCreate().shutdownBridgeOnEditorQuit)
                {
                    return;
                }
            }
            ...
            catch (Exception e)
            {
                ...
                UnityEngine.Debug.LogWarning(
                    $"[AIAssetPipeline] 에디터 종료 시 브리지 서버를 정리하지 못했습니다: {e.Message}");
            }
        }
        ...
        public static void Stop()
        {
            ...
            UnityEngine.Debug.Log($"[AIAssetPipeline] 브리지 서버를 종료했습니다 (PID {pid}).");
        }
        ...
        private static string BuildEarlyExitMessage(Process process, string python, int port, string logPath)
        {
            ...
            message.AppendLine("Python 실행 파일이 올바른지도 확인해주세요 (Tools/AI Asset Pipeline/Settings의 [자동 탐지] 버튼).");
            ...
            {
                message.AppendLine();
                message.AppendLine("설정의 \"브리지 콘솔 창 표시\"를 끄면 로그가 임시 폴더의 aiassetpipeline_bridge_server.log에 기록되어 원인을 확인할 수 있습니다.");
            }
