...
namespace AIAssetPipeline.Editor
{
    ...
    /// - aiassetpipeline_generate_candidates: 생성 Job 시작 (즉시 status:"started" 반환, 완료까지 대기하지 않음).
    /// - aiassetpipeline_list_candidates: Job 상태(running/completed/failed/idle)와 후보 목록 반환.
    /// - aiassetpipeline_select_candidate: 후보 확정 (창과 동일한 <see cref="CandidateGenerator.ConfirmCandidate"/> 공용 로직).
    /// - aiassetpipeline_delete_candidate: 후보 1개 삭제 (<see cref="CandidateGenerator.DeleteCandidate"/>).
    ...
    public static class ComfyUIGeneratorTool
    {
        ...
        static ComfyUIGeneratorTool()
        {
            AIAssetPipelineToolRegistry.Register(
                "aiassetpipeline_generate_candidates",
            ...
            AIAssetPipelineToolRegistry.Register(
                "aiassetpipeline_select_candidate",
                ...
                "파라미터: assetItemId(필수), candidatePath(필수, aiassetpipeline_list_candidates가 반환한 후보 경로), " +
                "scope(선택 — 생성 시 넘긴 scope와 같은 값. 미지정 시 기존 위치 그대로). " +
                "반환 data: { selectedPath }.",
                ExecuteSelect);

            AIAssetPipelineToolRegistry.Register(
                "aiassetpipeline_delete_candidate",
        ...
        private static object ExecuteGenerate(Dictionary<string, object> parameters)
        {
            ...
            if (Jobs.TryGetValue(jobKey, out existing) && existing.Status == "running")
            {
                throw new InvalidOperationException(
                    $"항목 \"{assetItemId}\"의 생성 Job이 이미 실행 중입니다. aiassetpipeline_list_candidates로 완료를 확인해주세요.");
            }
        ...
        private static async void RunJobAsync(
            GenerationJob job, AIAssetPipelineSettings settings, PromptItem item, string workflowName,
        ...
        private static object ExecuteList(Dictionary<string, object> parameters)
        {
            ...
            else
            {
                ...
                message = candidates.Count > 0
                    ...
                    : "생성된 후보가 없습니다. aiassetpipeline_generate_candidates로 먼저 생성하세요.";
            }
