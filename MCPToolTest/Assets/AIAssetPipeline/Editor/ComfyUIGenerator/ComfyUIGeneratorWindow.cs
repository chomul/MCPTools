...
{
    /// <summary>
    /// 3단계 생성 창입니다. (메뉴: Tools/AI Asset Pipeline/3. ComfyUI Generator)
    ...
    public class ComfyUIGeneratorWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/AI Asset Pipeline/3. ComfyUI Generator", false, 3)]
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (_bridgeAlive && !ownProcess)
                {
                    EditorGUILayout.HelpBox(
                        ...
                        "두 서버를 함께 쓰려면 Tools/AI Asset Pipeline/Settings에서 브리지 서버 주소의 포트를 다른 번호로 바꾸세요.",
                        MessageType.Info);
                }
        ...
        private void DrawInputSection()
        {
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    {
                        EditorGUILayout.HelpBox(
                            $"{AIAssetPipelineFolders.PromptSetDir(_settings)} 폴더에 PromptSet_*.json이 없습니다. " +
                            "2단계(Tools/AI Asset Pipeline/2. Prompt Builder)에서 먼저 저장해주세요.", MessageType.Warning);
                    }
                    else
                    {
        ...
        private void DrawGenerateSection()
        {
            ...
            // Tools/AI Asset Pipeline/Settings까지 가지 않고 이 창에서 바로 바꿀 수 있게 노출한다.
            EditorGUI.BeginChangeCheck();
            ...
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(_settings, "AI Asset Pipeline 후보 개수 변경");
            ...
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(_settings, "AI Asset Pipeline 설정 변경");
        ...
        private async Task TryFreeMemoryAsync()
        {
            ...
            try
            {
                ...
                Debug.Log("[AIAssetPipeline] 생성 완료 후 ComfyUI 모델을 언로드했습니다.");
            }
            catch (Exception e)
            {
                Debug.LogWarning($"[AIAssetPipeline] 생성 후 모델 언로드에 실패했습니다 (생성 결과에는 영향 없음): {e.Message}");
            }
        }
        ...
        private void MoveToNextUnconfirmedItem()
        {
            ...
            EditorUtility.DisplayDialog("ComfyUI 생성",
                "모든 항목이 확정되었습니다.\n4단계(Tools/AI Asset Pipeline/4. Asset Applier)로 진행하세요.", "확인");
        }
        ...
        private void RefreshPromptSetPaths()
        {
            ...
            _promptSetPaths = AIAssetPipelineFolders.FindDocuments(
                AIAssetPipelineFolders.DocsRoot(_settings), AIAssetPipelineFolders.PromptSetFolder, "PromptSet_*.json");
        }
