// MCP for Unity(com.coplaydev.unity-mcp) 패키지 의존은 이 어셈블리(AIAssetPipeline.Editor.McpForUnity)에 격리한다(어댑터 패턴).
// 패키지가 설치된 프로젝트에서만 versionDefines로 AIASSETPIPELINE_HAS_MCPFORUNITY 심볼이 정의되고,
// asmdef의 defineConstraints가 이 심볼을 요구하므로 패키지가 없는 배포 대상 프로젝트에서는
// 어셈블리 자체가 컴파일 대상에서 제외된다(참조 해석 전에 제외되므로 참조 누락 오류도 발생하지 않는다).
// 의존 방향은 McpForUnity → AIAssetPipeline.Editor 단방향이며, 본체는 이 어댑터를 호출하지 않는다.
#if AIASSETPIPELINE_HAS_MCPFORUNITY
...
{
    /// <summary>
    /// MCP for Unity 브리지에 AIAssetPipeline 도구를 노출하는 어댑터입니다.
    /// 파라미터(JObject)를 JSON 문자열로 변환해 <see cref="AIAssetPipelineToolRegistry.Execute"/>에 위임하고,
    ...
    internal static class McpForUnityAdapter
    {
        ...
        /// </summary>
        /// <param name="toolName">AIAssetPipelineToolRegistry에 등록된 도구 이름.</param>
        ...
        internal static object Handle(string toolName, JObject @params)
        {
            ...
            string resultJson = AIAssetPipelineToolRegistry.Execute(toolName, paramsJson);
            return JObject.Parse(resultJson);
        }
    }

    /// <summary>
    /// AI Asset Pipeline 연결 진단 도구(aiassetpipeline_ping)를 MCP for Unity에 노출합니다.
    /// 버전·Unity 버전·ComfyUI 서버 주소를 반환합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_ping",
        Description = "AI Asset Pipeline 연결 진단용 도구. 버전, Unity 버전, ComfyUI 서버 주소를 반환합니다.")]
    public static class AIAssetPipelinePingTool
    {
        ...
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_ping", @params);
        }
    }

    /// <summary>
    /// 1단계 분석 입력 수집 도구(aiassetpipeline_asset_scan)를 MCP for Unity에 노출합니다.
    /// 기획서 원문·프리팹 슬롯 스캔 결과·항목 스키마·작성 지침을 반환하며, AI가 이를 분석해 목록을 작성합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_asset_scan",
        Description = "1단계 에셋 리스트업 분석 입력 수집. 기획서 원문, 프리팹 슬롯 스캔 결과, itemSchema, " +
                      "instructions를 반환하며 AI가 분석해 목록 작성 후 aiassetpipeline_asset_list_save로 저장합니다. " +
    ...
    public static class AIAssetPipelineAssetScanTool
    {
        ...
        public class Parameters
        {
            ...
            [ToolParameter("true면 기획서에서 항목을 추출하지 않고 현재 열린 씬과 그 씬에 포함된 프리팹만 스캔해, " +
                           "대상 경로가 채워진 완성 items를 함께 반환합니다 (aiassetpipeline_asset_list_save에 그대로 전달 가능). " +
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_asset_scan", @params);
        }
    }

    /// <summary>
    /// 1단계 목록 저장 도구(aiassetpipeline_asset_list_save)를 MCP for Unity에 노출합니다.
    /// AI가 작성한 items 배열을 검증 후 에셋 목록 JSON으로 저장합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_asset_list_save",
    ...
    public static class AIAssetPipelineAssetListSaveTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
            /// <summary>저장할 에셋 목록 항목 배열 (itemSchema 형식의 객체 배열).</summary>
            [ToolParameter("저장할 에셋 목록 항목 배열. aiassetpipeline_asset_scan이 반환한 itemSchema 형식의 객체 배열.", Required = true)]
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_asset_list_save", @params);
        }
    }

    /// <summary>
    /// 2단계 프롬프트 재료 수집 도구(aiassetpipeline_prompt_scan)를 MCP for Unity에 노출합니다.
    /// 1단계 목록 항목·템플릿·프롬프트 스키마·작성 지침을 반환하며, AI가 이를 참고해 프롬프트를 작성합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_prompt_scan",
        Description = "2단계 프롬프트 제작 입력 수집. 1단계 에셋 목록 항목(assetItems), 프롬프트 템플릿(template), " +
                      ...
                      "aiassetpipeline_prompt_save로 저장합니다. " +
                      "파라미터: assetListPath(필수, Assets/ 상대 AssetList JSON), templateName(선택, 기본 default).")]
    public static class AIAssetPipelinePromptScanTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_prompt_scan", @params);
        }
    }

    /// <summary>
    /// 2단계 프롬프트 저장 도구(aiassetpipeline_prompt_save)를 MCP for Unity에 노출합니다.
    /// AI가 작성한 items 배열을 검증 후 PromptSet JSON으로 저장합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_prompt_save",
    ...
    public static class AIAssetPipelinePromptSaveTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
            /// <summary>저장할 프롬프트 항목 배열 (promptSchema 형식의 객체 배열).</summary>
            [ToolParameter("저장할 프롬프트 항목 배열. aiassetpipeline_prompt_scan이 반환한 promptSchema 형식의 객체 배열.", Required = true)]
            public List<object> items { get; set; }
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_prompt_save", @params);
        }
    }

    /// <summary>
    /// 3단계 후보 생성 도구(aiassetpipeline_generate_candidates)를 MCP for Unity에 노출합니다.
    /// 생성 Job을 시작하고 즉시 반환하며, 완료 여부는 aiassetpipeline_list_candidates로 폴링합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_generate_candidates",
        Description = "3단계 후보 생성 Job을 브리지 서버 경유로 시작합니다 (비동기, 완료까지 기다리지 않음). 기준 시드부터 " +
                      ...
                      "완료 여부와 결과는 aiassetpipeline_list_candidates로 폴링하세요. " +
    ...
    public static class AIAssetPipelineGenerateCandidatesTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
            ...
            /// <see cref="AIAssetPipelineGenerateCandidatesTool.HandleCommand"/>가 받은 <c>JObject</c>를 그대로
            ...
            [ToolParameter("PromptSet 단위 저장 스코프 (보통 PromptSet 파일명에서 확장자를 뗀 값, 예: PromptSet_20260721_0512). " +
                           ...
                           "이후 aiassetpipeline_list_candidates/aiassetpipeline_select_candidate에도 같은 값을 넘기세요. 생략 시 기존 위치 그대로.",
                Required = false)]
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_generate_candidates", @params);
        }
    }

    /// <summary>
    /// 3단계 후보 조회 도구(aiassetpipeline_list_candidates)를 MCP for Unity에 노출합니다.
    /// 생성 Job 상태와 후보 목록을 반환합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_list_candidates",
    ...
    public static class AIAssetPipelineListCandidatesTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
            ...
            /// <summary>PromptSet 단위 저장 스코프.</summary>
            [ToolParameter("PromptSet 단위 저장 스코프 (aiassetpipeline_generate_candidates에 넘긴 scope와 같은 값). " +
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_list_candidates", @params);
        }
    }

    /// <summary>
    /// 3단계 후보 확정 도구(aiassetpipeline_select_candidate)를 MCP for Unity에 노출합니다.
    /// 선택한 후보를 확정 경로로 복사하고 임포트 설정을 적용합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_select_candidate",
    ...
    public static class AIAssetPipelineSelectCandidateTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
            ...
            [ToolParameter("확정할 후보 파일 경로 (aiassetpipeline_list_candidates가 반환한 path).", Required = true)]
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_select_candidate", @params);
        }
    }

    /// <summary>
    /// 3단계 후보 삭제 도구(aiassetpipeline_delete_candidate)를 MCP for Unity에 노출합니다.
    /// 후보 파일 1개를 .meta와 같은 시드의 메타 JSON까지 함께 삭제합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_delete_candidate",
        Description = "3단계 후보 1개를 삭제합니다 (파일 + .meta + 같은 시드의 메타 JSON). " +
                      "후보 폴더(3_Candidates/, 구 Candidates/) 밖의 경로는 안전장치로 거부합니다. " +
                      "파라미터: assetItemId(필수), candidatePath(필수, aiassetpipeline_list_candidates가 반환한 path). " +
                      "반환 data: { deletedPath, assetItemId }.")]
    public static class AIAssetPipelineDeleteCandidateTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
            ...
            [ToolParameter("삭제할 후보 파일 경로 (aiassetpipeline_list_candidates가 반환한 path, Assets/ 기준 상대 경로).", Required = true)]
            public string candidatePath { get; set; }
        }
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_delete_candidate", @params);
        }
    }

    /// <summary>
    /// 4단계 단건 적용 도구(aiassetpipeline_apply_asset)를 MCP for Unity에 노출합니다.
    /// AssetList 항목 1개의 확정본을 대상 프리팹의 컴포넌트에 적용합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_apply_asset",
    ...
    public static class AIAssetPipelineApplyAssetTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_apply_asset", @params);
        }
    }

    /// <summary>
    /// 4단계 일괄 적용 도구(aiassetpipeline_apply_all)를 MCP for Unity에 노출합니다.
    /// AssetList 전체 항목을 순차 적용하고, 실패 항목은 사유와 함께 반환합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_apply_all",
    ...
    public static class AIAssetPipelineApplyAllTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_apply_all", @params);
        }
    }

    /// <summary>
    /// 파이프라인 후반부 자동화 도구(aiassetpipeline_run_pipeline)를 MCP for Unity에 노출합니다.
    /// PromptSet(2단계 산출물)부터 3단계 생성 → (autoSelect="first"면) 확정 → 4단계 적용을 Job으로 실행하며,
    /// 호출은 즉시 status:"started"로 반환하고 진행·결과는 aiassetpipeline_status의 pipeline 블록으로 조회합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_run_pipeline",
        Description = "파이프라인 후반부 자동화 Job을 시작합니다 (비동기, 완료까지 기다리지 않음). PromptSet(2단계 산출물, " +
                      ...
                      "(이후 aiassetpipeline_select_candidate + aiassetpipeline_apply_asset). " +
                      ...
                      "진행 상황과 최종 결과(pendingSelections/applied/failed)는 aiassetpipeline_status의 pipeline 블록으로 폴링하세요. " +
                      "Job은 동시에 1개만 실행할 수 있습니다.")]
    public static class AIAssetPipelineRunPipelineTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_run_pipeline", @params);
        }
    }

    /// <summary>
    /// 파이프라인 진단 도구(aiassetpipeline_status)를 MCP for Unity에 노출합니다.
    /// 설정값·산출물 현황·버전과 aiassetpipeline_run_pipeline Job의 진행 상태(pipeline 블록)를 반환합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_status",
        Description = "AI Asset Pipeline 진단: 설정값(ComfyUI/브리지 주소, 경로, 후보 개수), 산출물 현황(AssetList/PromptSet 개수·최신 파일, " +
                      "3_Confirmed/Images·Audio 및 3_Candidates 개수, 확정 항목 수), 버전·Unity 버전, " +
                      "그리고 aiassetpipeline_run_pipeline Job의 진행 상태를 반환합니다. " +
    ...
    public static class AIAssetPipelineStatusTool
    {
        /// <summary>MCP for Unity 브리지가 리플렉션으로 호출하는 핸들러입니다.</summary>
        /// <param name="params">파라미터 JSON 객체 (사용하지 않음).</param>
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_status", @params);
        }
    }

    /// <summary>
    /// 스프라이트 시트 프롬프트 조립 도구(aiassetpipeline_spritesheet_build_prompt)를 MCP for Unity에 노출합니다.
    /// 멀티 행(동작별 Row) 통합 시트 프롬프트(영어)를 조립하고 JSON으로 저장합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_spritesheet_build_prompt",
        Description = "레퍼런스 이미지 첨부 전제의 멀티 행(동작별 Row) 통합 스프라이트 시트 프롬프트(영어)를 조립하고 " +
                      ...
                      "외부 AI(이미지 생성)에 붙여넣어 시트를 만든 뒤 aiassetpipeline_spritesheet_import로 가져옵니다. " +
    ...
    public static class AIAssetPipelineSpriteSheetBuildPromptTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_spritesheet_build_prompt", @params);
        }
    }

    /// <summary>
    /// 스프라이트 시트 임포트 도구(aiassetpipeline_spritesheet_import)를 MCP for Unity에 노출합니다.
    /// 외부 AI가 만든 멀티 행 시트 png를 격자선 기준으로 슬라이스하고 Sprite Multiple을 적용합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_spritesheet_import",
    ...
    public static class AIAssetPipelineSpriteSheetImportTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_spritesheet_import", @params);
        }
    }

    /// <summary>
    /// 스프라이트 시트 클립 생성 도구(aiassetpipeline_spritesheet_build_clips)를 MCP for Unity에 노출합니다.
    /// 슬라이스된 시트의 서브 스프라이트를 동작별로 묶어 AnimationClip을 만들고,
    /// 옵션에 따라 AnimatorController 구성 + 대상 프리팹 연결까지 수행합니다.
    /// </summary>
    [McpForUnityTool("aiassetpipeline_spritesheet_build_clips",
    ...
    public static class AIAssetPipelineSpriteSheetBuildClipsTool
    {
        /// <summary>MCP for Unity 스키마 생성용 파라미터 정의입니다(리플렉션으로 읽힘).</summary>
        public class Parameters
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            return McpForUnityAdapter.Handle("aiassetpipeline_spritesheet_build_clips", @params);
        }
    }
}
#endif
