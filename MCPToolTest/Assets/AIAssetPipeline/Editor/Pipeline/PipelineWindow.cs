...
{
    /// <summary>
    /// 4단계 파이프라인 통합 안내 창입니다. (메뉴: Tools/AI Asset Pipeline/Pipeline (All-in-One))
    ...
    public class PipelineWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/AI Asset Pipeline/Pipeline (All-in-One)", false, 20)]
        public static void Open()
        {
            var window = GetWindow<PipelineWindow>();
            window.titleContent = new GUIContent("AI Asset Pipeline");
        ...
        private string FindLatest(string prefix)
        {
            string subfolder = prefix == "AssetList_"
                ? AIAssetPipelineFolders.AssetListFolder
                : AIAssetPipelineFolders.PromptSetFolder;

            return AIAssetPipelineFolders
                .FindDocuments(AIAssetPipelineFolders.DocsRoot(_settings), subfolder, prefix + "*.json")
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    AIAssetPipelineSettingsWindow.Open();
                }
            }
            ...
            EditorGUILayout.HelpBox(
                "MCP 도구로 후반부(3~4단계)를 자동화하려면 aiassetpipeline_run_pipeline(promptSetPath, autoSelect) 를 사용하세요. " +
                "설정·산출물 현황 진단은 aiassetpipeline_status 로 확인할 수 있습니다.",
                MessageType.None);
