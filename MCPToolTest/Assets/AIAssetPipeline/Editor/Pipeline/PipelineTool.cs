...
namespace AIAssetPipeline.Editor
{
    ...
    /// - aiassetpipeline_run_pipeline: 2단계 산출물(PromptSet)부터 3단계 후보 생성 → (autoSelect="first"면) 확정 → 4단계 적용을
    ///   Job으로 실행합니다(즉시 status:"started" 반환, 완료까지 기다리지 않음).
    /// - aiassetpipeline_status: 설정값·산출물 현황·버전에 더해 파이프라인 Job의 진행 상태(pipeline 블록)를 반환.
    ///
    ...
    /// <see cref="RunPipelineJobAsync"/>(async void)에서 진행하며, 진행 상황과 최종 결과는 aiassetpipeline_status의
    /// pipeline 블록으로 조회한다. Job은 동시에 1개만 허용한다.
    ///
    /// [메인 스레드 보장 근거]
    /// MCP 핸들러는 <see cref="AIAssetPipelineToolRegistry.Execute"/>를 통해 에디터 메인 스레드에서 호출되므로
    ...
    public static class PipelineTool
    {
        ...
        static PipelineTool()
        {
            AIAssetPipelineToolRegistry.Register(
                ...
                "(이후 aiassetpipeline_select_candidate + aiassetpipeline_apply_asset으로 진행). " +
            ...
            AIAssetPipelineToolRegistry.Register(
                ...
                "AI Asset Pipeline 진단: 설정값(ComfyUI/브리지 주소, 경로, 후보 개수), 산출물 현황(AssetList/PromptSet 개수·최신 파일, " +
        ...
        private static object ExecuteRunPipeline(Dictionary<string, object> parameters)
        {
            ...
            if (_job != null && _job.Status == "running")
            {
                throw new InvalidOperationException(
                    ...
                    "동시에 1개만 실행할 수 있습니다. aiassetpipeline_status의 pipeline 블록으로 완료를 확인한 뒤 다시 호출해주세요 " +
                    $"(응답이 없으면 남은 타임아웃 {Math.Max(0, _job.TimeoutSeconds - (int)(DateTime.UtcNow - _job.StartedAtUtc).TotalSeconds)}초 뒤 자동 실패 처리됩니다).");
            }

            var settings = AIAssetPipelineSettings.GetOrCreate();
            ...
            return new Dictionary<string, object>
            {
                ...
                { "timeoutSeconds", timeoutSeconds },
                { "statusNote", "진행 상황과 결과(pendingSelections/applied/failed)는 aiassetpipeline_status의 pipeline 블록으로 확인하세요." }
            };
        }
        ...
        private static async void RunPipelineJobAsync(
            PipelineJob job, AIAssetPipelineSettings settings, List<PromptItem> items, string workflowName)
        {
        ...
        private static void EnsureMainThread(PipelineJob job)
        {
            if (Thread.CurrentThread.ManagedThreadId != job.MainThreadId)
            {
                throw new InvalidOperationException(
                    ...
                    "후보 파일은 디스크에 남아 있으므로 aiassetpipeline_list_candidates로 확인 후 " +
                    "aiassetpipeline_select_candidate/aiassetpipeline_apply_asset으로 이어서 진행해주세요.");
            }
        }
        ...
        private static void ApplyConfirmed(
            AIAssetPipelineSettings settings, string assetListPath, Dictionary<string, string> confirmed,
        ...
        private static object ExecuteStatus(Dictionary<string, object> parameters)
        {
            ...
            string docs = AIAssetPipelineFolders.DocsRoot(settings);
            string root = AIAssetPipelineFolders.GeneratedRoot(settings);
            string confirmed = AIAssetPipelineFolders.ConfirmedRoot(settings);

            // 단계별 하위 폴더 도입 이전 위치의 산출물도 함께 센다.
            string[] assetLists = AIAssetPipelineFolders.FindDocuments(docs, AIAssetPipelineFolders.AssetListFolder, "AssetList_*.json");
            string[] promptSets = AIAssetPipelineFolders.FindDocuments(docs, AIAssetPipelineFolders.PromptSetFolder, "PromptSet_*.json");
            ...
            var outputs = new Dictionary<string, object>
            {
                ...
                { "candidateFolderCount",
                    CountSubfolders($"{root}/{AIAssetPipelineFolders.CandidatesFolder}") +
                    CountSubfolders($"{root}/{AIAssetPipelineFolders.LegacyCandidatesFolder}") },
            ...
            return new Dictionary<string, object>
            {
                { "version", AIAssetPipelineInfo.Version },
                { "unityVersion", Application.unityVersion },
                { "config", config },
                { "outputs", outputs },
                { "pipeline", BuildPipelineStatus() },
                { "serverHealthNote",
                    "서버 실시간 연결 확인은 이 도구가 수행하지 않습니다. Tools/AI Asset Pipeline/3. ComfyUI Generator 창의 서버 상태 " +
                    "또는 브리지 서버 /health를 참조하세요." }
            };
        }

        /// <summary>
        /// aiassetpipeline_run_pipeline Job의 진행 상태를 반환합니다 (Job 기록이 없으면 status:"idle").
        ...
        private static Dictionary<string, object> BuildPipelineStatus()
        {
            ...
            if (job == null)
            {
                ...
                {
                    { "status", "idle" },
                    { "message", "이 세션에서 실행한 파이프라인 Job이 없습니다. aiassetpipeline_run_pipeline으로 시작하세요." }
                };
            }
