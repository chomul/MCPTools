...
namespace AIAssetPipeline.Editor
{
    ...
    internal static class ApplyHistory
    {
        ...
        internal static void Record(AssetListItem item, ApplyResult result)
        {
            ...
            {
                Debug.LogWarning(
                    $"[AIAssetPipeline] 적용 이력을 기록하지 못했습니다 (항목 \"{item.id}\"): {e.Message} " +
        ...
        /// </returns>
        internal static HashSet<string> LoadAppliedItemIds(AIAssetPipelineSettings settings)
        {
            ...
            {
                Debug.LogWarning($"[AIAssetPipeline] 적용 이력을 읽지 못해 빈 이력으로 진행합니다: {e.Message}");
            }
