...
namespace AIAssetPipeline.Editor
{
    ...
    /// - aiassetpipeline_apply_asset: 항목 1개 적용 (assetPath 생략 시 확정본 자동 탐색).
    /// - aiassetpipeline_apply_all: 목록 전체 일괄 적용 (검증 실패 항목은 failed에 사유와 함께 반환).
            ...
            foreach (AssetListItem item in doc.items)
            {
                ...
                if (string.IsNullOrEmpty(confirmedPath))
                {
                    failed.Add(MakeFailed(item.id,
                        "확정본을 찾을 수 없습니다. 3단계(aiassetpipeline_select_candidate)에서 먼저 확정해주세요."));
                    continue;
                }
