...
namespace AIAssetPipeline.Editor
{
    ...
    public sealed class ComfyUIClient : IDisposable
    {
        ...
        private readonly AIAssetPipelineSettings _settings;
        ...
        public ComfyUIClient(AIAssetPipelineSettings settings)
        {
            ...
            if (!Uri.TryCreate(_serverUrl, UriKind.Absolute, out baseUri))
            {
                throw new InvalidOperationException(
                    ...
                    "Tools/AI Asset Pipeline/Settings에서 주소를 확인해주세요. (예: http://127.0.0.1:8188)");
            }
            ...
            while (true)
            {
                ...
                if (elapsed > timeoutSeconds)
                {
                    throw new TimeoutException(
                        ...
                        "서버 부하 상태를 확인하거나 Tools/AI Asset Pipeline/Settings에서 타임아웃을 늘려주세요.");
                }
        ...
        private string BuildConnectionErrorMessage()
        {
            return $"ComfyUI 서버({_serverUrl})에 연결할 수 없습니다. " +
                   "ComfyUI를 실행한 뒤 Tools/AI Asset Pipeline/Settings에서 연결 테스트를 해주세요.";
        }
    }
