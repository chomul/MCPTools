...
{
    /// <summary>
    /// AI Asset Pipeline 파이프라인 전역 설정을 담는 ScriptableObject입니다.
    /// <see cref="GetOrCreate"/>로 로드하며, 에셋이 없으면 기본값으로 자동 생성됩니다.
    /// <see cref="InstallRoot"/>로 AIAssetPipeline 폴더의 실제 설치 위치를 알 수 있으므로
    /// 도입 프로젝트가 폴더를 어디에 두든 동작합니다.
    /// </summary>
    public class AIAssetPipelineSettings : ScriptableObject
    {
        /// <summary>설치 루트를 찾지 못했을 때 사용하는 표준 설치 위치입니다.</summary>
        private const string DefaultInstallRoot = "Assets/AIAssetPipeline";
        ...
        /// </summary>
        private const string UserAssetPath = "Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset";
        ...
        /// </summary>
        private static AIAssetPipelineSettings _cached;

        /// <summary>
        /// AIAssetPipeline 폴더의 설치 루트 경로입니다
        /// (에셋 경로 기준, 예: "Assets/AIAssetPipeline" 또는 UPM 설치 시 "Packages/&lt;패키지명&gt;").
        /// 이 스크립트 파일의 실제 위치에서 런타임에 계산하므로, 도입 프로젝트가 폴더를
        /// 다른 위치(예: "Assets/Plugins/AIAssetPipeline")에 두거나 UPM 패키지로 설치해도 동작합니다.
        /// 확인할 수 없으면 표준 위치인 "Assets/AIAssetPipeline"로 폴백합니다.
        /// </summary>
        ...
        /// <summary>
        /// 설정 에셋을 새로 생성할 때 사용하는 경로입니다 (고정: "Assets/AIAssetPipeline.User/AIAssetPipelineSettings.asset").
        ...
        /// false(기본)면 창 없이 실행되고 로그는 시스템 임시 폴더의 aiassetpipeline_bridge_server.log에 기록됩니다.
        /// </summary>
        ...
        /// 없을 때만 <see cref="AssetPath"/>("Assets/AIAssetPipeline.User/")에 기본값으로 새로 생성한 뒤 저장합니다.
        /// 조회 범위를 Assets로 한정하므로 패키지(Packages/)에 동봉된 설정 에셋이
        /// 사용자 설정을 가리지 않습니다.
        /// </summary>
        /// <returns>로드되었거나 새로 생성된 <see cref="AIAssetPipelineSettings"/> 인스턴스.</returns>
        public static AIAssetPipelineSettings GetOrCreate()
        {
            ...
            if (guids != null && guids.Length > 0)
            {
                ...
                var found = AssetDatabase.LoadAssetAtPath<AIAssetPipelineSettings>(foundPath);
                if (found != null)
                {
                    if (guids.Length > 1)
                    {
                        ...
                        Debug.LogWarning(
                            $"[AIAssetPipeline] 설정 에셋이 {guids.Length}개 발견되어 첫 번째(\"{foundPath}\")를 사용합니다. " +
            ...
            var settings = CreateInstance<AIAssetPipelineSettings>();

            // 새 에셋은 처음부터 최신 버전으로 저장해 마이그레이션이 돌지 않게 한다.
            settings.settingsVersion = CurrentSettingsVersion;
            AssetDatabase.CreateAsset(settings, assetPath);
            AssetDatabase.SaveAssets();
            Debug.Log($"[AIAssetPipeline] 설정 에셋을 새로 생성했습니다: {assetPath}");
            _cached = settings;
            return settings;
        }
        ...
        private static void Migrate(AIAssetPipelineSettings settings, string assetPath)
        {
            ...
            {
                Debug.Log(
                    $"[AIAssetPipeline] 설정 에셋(\"{assetPath}\")의 \"일괄 생성 후 모델 언로드\"를 껐습니다. " +
                    "연속 생성 시 회차마다 붙던 모델 재로드 대기(약 10~40초)를 없애기 위한 1회 보정입니다. " +
                    "VRAM이 부족해 다시 켜려면 Tools/AI Asset Pipeline/Settings에서 \"일괄 생성 후 모델 언로드\"를 체크하세요.");
            }
        }
        ...
        {
            // ① 이 스크립트 파일(Editor/Common/AIAssetPipelineSettings.cs) 위치에서 역산한다.
            //    Assets/ 설치든 UPM 패키지(Packages/<패키지명>/...) 설치든 실제 경로가 그대로 나온다.
            string scriptPath = string.Empty;
            var probe = CreateInstance<AIAssetPipelineSettings>();
            try
            {
            ...
            //    (새 설정 에셋은 설치 루트 밖 "Assets/AIAssetPipeline.User/"에 생성되므로 역산 근거로는 부적합하지만,
            //     구버전 설치본의 "<루트>/Editor/Common/AIAssetPipelineSettings.asset" 배치를 위해 유지한다.)
            foreach (string guid in AssetDatabase.FindAssets("t:AIAssetPipelineSettings", new[] { "Assets" }))
            {
