...
// 별도 어셈블리(AIAssetPipeline.Editor.McpForUnity)에서 처리한다. 패키지 API 의존은 그 어셈블리에만
...
namespace AIAssetPipeline.Editor
{
    ...
    /// </summary>
    [InitializeOnLoad]
    public static class AIAssetPipelineToolRegistry
    {
        ...
        /// 확실한 것만 허용한다.
        /// - aiassetpipeline_ping: 설정값·버전 문자열만 반환한다.
        /// - aiassetpipeline_status: 설정값과 산출물 폴더의 파일 개수를 세어 반환한다 (Directory 열거만 사용).
        ///   여기에 더해 aiassetpipeline_run_pipeline Job의 진행 상태(pipeline 블록)를 메모리에서 읽어 담을 뿐이므로
        ///   여전히 부작용이 없다. 오히려 Job이 도는 동안 에이전트가 폴링하는 경로라 반드시 허용해야 한다.
        /// - aiassetpipeline_list_candidates: 후보 폴더를 열거해 Job 상태와 후보 목록만 반환한다. 생성 Job이
        ///   도는 동안 에이전트가 폴링하는 경로이므로 Play Mode에서도 막지 않는다.
        /// 세 도구 모두 <see cref="AIAssetPipelineSettings.GetOrCreate"/>를 거치므로 설정 에셋이 아직 없는
        /// 프로젝트에서 최초 1회만 설정 에셋을 만든다(도메인 리로드를 유발하지 않는 부트스트랩).
        ///
        /// 나머지 12개 도구는 파일 저장·에셋 임포트·프리팹 저장·씬 열기·async 생성 시작 중 하나 이상을
        /// 수행하므로 화이트리스트에 넣지 않는다 (aiassetpipeline_prompt_scan은 파일 읽기만 하지만,
        ...
        {
            "aiassetpipeline_ping",
            "aiassetpipeline_status",
            "aiassetpipeline_list_candidates"
        };

        static AIAssetPipelineToolRegistry()
        {
            Register(
                "aiassetpipeline_ping",
                "AI Asset Pipeline 연결 진단용 도구. 버전·Unity 버전·ComfyUI 서버 주소를 반환합니다.",
                _ =>
                {
        ...
        /// </summary>
        /// <param name="toolName">도구 이름 (예: "aiassetpipeline_ping").</param>
        ...
        /// <summary>로컬 검증용: aiassetpipeline_ping을 실행해 결과를 콘솔에 출력합니다.</summary>
        [MenuItem("Tools/AI Asset Pipeline/Ping (Local Test)", false, 101)]
        private static void PingLocalTest()
        {
            string result = Execute("aiassetpipeline_ping", "{}");
            Debug.Log($"[AIAssetPipeline] aiassetpipeline_ping 결과: {result}");
        }
    }
}
