...
{
    /// <summary>
    /// AI Asset Pipeline 설정 편집 + ComfyUI 서버 연결 테스트 창입니다. (메뉴: Tools/AI Asset Pipeline/Settings)
    /// </summary>
    public class AIAssetPipelineSettingsWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/AI Asset Pipeline/Settings", false, 100)]
        public static void Open()
        {
            var window = GetWindow<AIAssetPipelineSettingsWindow>();
            window.titleContent = new GUIContent("AI Asset Pipeline 설정");
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"AI Asset Pipeline v{AIAssetPipelineInfo.Version}", EditorStyles.boldLabel);
        ...
        private void DrawSettingsFields()
        {
            ...
            bool showConsole = EditorGUILayout.Toggle(
                new GUIContent("브리지 콘솔 창 표시",
                    ...
                    "끄면(기본) 창 없이 실행되고 로그는 임시 폴더의 aiassetpipeline_bridge_server.log에 기록됩니다."),
        ...
        private void DetectPythonExecutable()
        {
            ...
            Undo.RecordObject(_settings, "AI Asset Pipeline Python 실행 파일 자동 탐지");
            _settings.pythonExecutable = python;
            EditorUtility.SetDirty(_settings);
            AssetDatabase.SaveAssets();
        ...
        /// 사용자 오버라이드 폴더("Assets/AIAssetPipeline.User/ComfyUI/")로 복사합니다.
        ...
        private void DrawRegisteredTools()
        {
            ...
            foreach (string toolName in AIAssetPipelineToolRegistry.ToolNames)
            {
        ...
        private void ApplyDetectedPort(int port)
        {
            ...
            Undo.RecordObject(_settings, "AI Asset Pipeline ComfyUI 주소 자동 감지");
