...
{
    /// <summary>
    /// AI Asset Pipeline가 사용하는 프로젝트 작업 폴더(기획서 문서 루트, 생성 결과 루트)를 보장하는 헬퍼입니다.
    /// 도구를 처음 넣은 프로젝트에서 사용자가 폴더를 직접 만들지 않아도 되도록
    /// 각 단계 창이 열릴 때 호출합니다 (D12).
    /// </summary>
    internal static class AIAssetPipelineFolders
    {
        // ──────────────────── 단계별 하위 폴더 이름 ────────────────────
        ...
        internal static string DocsRoot(AIAssetPipelineSettings settings)
        {
        ...
        internal static string GeneratedRoot(AIAssetPipelineSettings settings)
        {
        ...
        /// </summary>
        internal static string AnimationsDir(AIAssetPipelineSettings settings)
        {
        ...
        internal static void EnsureWorkFolders(AIAssetPipelineSettings settings)
        {
            if (settings == null)
            {
                return;
            ...
            if (created.Count > 0)
            {
                Debug.Log($"[AIAssetPipeline] 작업 폴더가 없어 새로 만들었습니다: {string.Join(", ", created)}");
            }
        }
