...
- [x] (2026-07-30 확정·중요) **격자선이 셀마다 배경을 가두던 문제 수정** — v0.6.0 배포 후 사용자 제보(`Sprite_Item003.png`, `Sprite_Item007.png`). v0.6.0의 램프 통과는 "밝아지는 방향"만 허용해서 두 가지 형태를 놓쳤다. ① Item003: 이미지 가장자리가 흰 여백이고 안쪽에 격자선(실측 `253 → 230 → 207 → 250`, **어두워지는 방향**) → 0.8%만 제거. ② Item007: 격자선 자체가 근사 흰색보다 훨씬 어두움(실측 min 채널 **133~180**, 2px 폭) → 외곽에서 닿는 셀만 지워지고 나머지 셀은 흰 배경 유지(44.9%). 임계값을 133까지 낮추는 방식은 회색조 캐릭터 몸통을 통째로 먹을 위험이 있어 채택하지 않고, **얇은 장벽만 건너뛰는** 규칙으로 해결: 배경 픽셀에서 한 방향으로 최대 `MaxBarrierBridgeThickness`(4px)까지 **무채색만** 지나 근사 흰색에 닿으면 그 사이를 편입. 4px보다 두꺼운 무채색 덩어리는 근사 흰색에 닿지 못해 통과 불가, 유채색 장벽은 두께 무관 통과 불가. 추가로 건너뛴 픽셀을 `RestorePockets`의 **연결자**로 넘김 — 격자선은 얇아 침식하면 다리가 끊기고 셀 배경 전체가 고립 pocket으로 판정돼 되살아난다(실측 최대 -2.4%p).
  - 검증 상태: 실제 알고리즘(BFS + fringe + pocket restore)을 그대로 포팅해 시트 9종 실측 — Item003 0.8% → **56.2%**, Item007 44.9% → **73.0%**, 08_05_25 78.5% → 90.1%, 04_19_14 67.8% → 69.3%, 04_12_54 66.0% → 66.9%, 07_36_31 77.3% → 78.0%, 10_39_56 73.0% → 73.6%, 03_20_31·05_41_26 변화 없음. 추가 제거된 픽셀 중 **유채색 0개**. 연결자 적용 시 pocket restore 후에도 BFS 결과가 0.2%p 이내로 유지됨(미적용 시 최대 -2.9%p 손실). 제거 마스크 자홍색 미리보기로 두 제보 시트의 배경 완전 제거·스프라이트 무손상 육안 확인. Unity Roslyn으로 `MCPTools.Editor`/`.Tests` 클린 컴파일.
  - 관련 파일: MCPToolTest/Assets/MCPTools/Editor/SpriteSheet/SpriteSheetImporter.cs
...
- [ ] (2026-07-30) 제보 시트 `Sprite_Item003.png` / `Sprite_Item007.png`를 [배경 제거 + 격자 검출]에 넣으면 **모든 셀**의 흰 배경이 투명해짐 (일부 셀만 남는 현상 없음)
- [ ] (2026-07-30) 회색조·흑백 스프라이트 시트가 있으면 임포트해 캐릭터 몸통이 배경으로 먹히지 않는지 확인 (4px 초과 무채색 덩어리는 통과 불가 설계이나 실물 미검증)
