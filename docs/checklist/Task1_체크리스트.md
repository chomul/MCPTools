...
## 기존 목록 불러와 이어서 편집 (2026-07-25 요청)

- [x] `AssetListBuilder.Load(path)` 신설 — 저장한 `AssetList_*.json`을 `AssetListDocument`로 되읽는다. 항목 0개 문서도 오류로 보지 않는다(빈 목록을 저장했다 다시 채우는 흐름 허용). 파싱 실패/파일 없음은 각각 `FormatException`/`FileNotFoundException`.
  - `PromptBuilder.LoadAssetList`와 기능이 겹치지만 그쪽을 참조하면 AssetListup → PromptBuilder 의존이 생기므로, `Save`와 짝이 되는 위치인 `AssetListBuilder`에 별도로 둔다.
  - 관련 파일: `Editor/AssetListup/AssetListBuilder.cs`
- [x] 1단계 창 최상단에 **기존 목록 불러오기** 박스 추가 — `docsRootPath`의 `AssetList_*.json`을 최신순 드롭다운(파일명만 표시) + [불러오기] + [새로고침]. 저장된 목록이 없으면 안내 문구만.
  - 편집 중인 목록이 있으면 버릴지 확인한 뒤 교체한다(스캔 교체 흐름과 같은 규칙).
  - 불러온 뒤 항목 추가/수정/삭제는 기존 표 UI를 그대로 쓴다(신규 UI 없음).
- [x] [저장]이 불러온 파일을 편집 중이면 **덮어쓰기 / 새 파일로 저장 / 취소**를 묻는다(`DisplayDialogComplex`). 그 외에는 종전대로 타임스탬프 새 파일. 저장 후 방금 저장한 파일이 편집 대상으로 유지되어 반복 덮어쓰기가 가능하다.
- [x] 스캔·AI 생성이 `_document`를 통째로 교체하면 불러온 파일과의 연결이 끊긴다 — `_loadedDocument` 참조 비교(`IsEditingLoadedFile`)로 판정해, 관계 없는 파일에 덮어쓰기를 제안하지 않는다. AI "목록에 병합"처럼 같은 인스턴스를 수정하는 경우는 연결이 유지된다.
  - 관련 파일: `Editor/AssetListup/AssetListupWindow.cs`, `Assets/MCPTools/README.md`(1단계 §0-1)
- 검증 상태: 정적 검증(중괄호/괄호 균형, 참조 심볼 존재 확인). **Unity 컴파일·동작 확인 필요.**

### 에디터 테스트 (기존 목록 불러오기)

- [ ] 저장된 목록이 없는 프로젝트에서 창을 열면 "저장된 목록이 없습니다" 안내만 뜨고 [불러오기]가 비활성
- [ ] 목록을 저장한 뒤 [새로고침] → 드롭다운에 파일명이 **최신순**으로 나타남
- [ ] [불러오기] → 표에 항목이 채워지고 상태줄에 "불러오기 완료: 경로 (항목 N개)" 표시. 편집 중 목록이 있으면 버릴지 먼저 물음
- [ ] 불러온 뒤 항목 추가·셀 수정·[삭제] 후 [저장] → **덮어쓰기 / 새 파일로 저장 / 취소** 3지 선택이 뜨고, 덮어쓰기 시 같은 파일이 갱신됨(파일 개수 안 늘어남)
- [ ] [새 파일로 저장] 선택 시 새 타임스탬프 파일이 생기고, 이후 [저장]은 **새로 만든 파일**을 덮어쓰기 대상으로 제안함
- [ ] 불러온 뒤 [스캔 + 휴리스틱 추출] 등으로 목록을 교체하면 "편집 중:" 표시가 사라지고, [저장]이 덮어쓰기를 묻지 않고 새 파일로 저장됨
- [ ] 저장 직후 프로젝트 창에 파일이 즉시 보임(D13 회귀)
