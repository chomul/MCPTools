...
- [x] **[S1]** 모델 언로드 기본값·호출 위치 수정
  ...
  - [x] **연속 3회 생성 실측 (2026-07-26, 주 에이전트 · MCP `mcptools_generate_candidates`)** — 아래 "S1 실측" 참조

#### S1 실측 결과 — 목표 수치는 이 환경에서 재현되지 않음

환경: RTX 4070 12.9GB, ComfyUI 상시 기동, `GenerateImage` 워크플로, `candidateCount=1`, 브리지 0.4.0.

| 회차 | wall clock | ComfyUI `execution_start→success` |
|------|-----------|-----------------------------------|
| 1회차 | 130초 | 121.7초 |
| 2회차 (연속) | 131초 | 122.1초 |
| **3회차 (`POST /free`로 모델 언로드 직후)** | **118초** | **110.6초** |

- **마이그레이션은 정상 동작.** 설정 에셋이 `unloadModelsAfterBatch: 0` + `settingsVersion: 1`로 보정돼 있음(원래 v0 / `true`였던 에셋).
- **단건 경로의 `/free` 호출 제거 확인.** 코드베이스 전체에서 `TryFreeMemoryAsync()` 호출부는 `ComfyUIGeneratorWindow.cs:1873`(일괄 경로) **한 곳뿐**이고, MCP 경로에는 없다. 구조적으로 회차 간 언로드가 일어나지 않는다.
- **그러나 §4 S1이 주장한 "회차당 10~40초 절감"은 이 환경에서 관측되지 않았다.** 모델을 강제 언로드한 3회차가 오히려 **더 빨랐다**(110.6초 vs 121.7/122.1초). 즉 이 하드웨어·모델 조합에서 재로드 비용은 회차 간 편차(±10초) 안에 묻힌다 — 체크포인트가 OS 페이지 캐시에 있어 VRAM 재적재가 저렴한 것으로 보인다. 원 추정치는 SDXL/Flux 6~12GB 기준이었다.
- **함의**: S1 변경 자체는 옳다(불필요한 작업 제거). 다만 **§1 정량 목표의 "12~45초 → 3초 이하"는 대형 모델 환경에 한정된 값**으로 읽어야 하며, 이 프로젝트의 실제 워크플로에서는 생성 1건이 **약 120초**라 S1·S4·S5·S6가 다루는 오버헤드(합계 수 초)는 **전체의 2% 미만**이다. 속도 체감을 더 키우려면 워크플로 자체(steps·해상도·샘플러)나 후보 수가 대상이다.
- 미측정: VRAM이 작은 환경에서의 OOM 여부(§8 D-16) — 12.9GB 환경만 확인.
